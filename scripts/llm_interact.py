...
GEMINI_MODEL_FLASH = 'gemini-2.5-flash-preview-04-17' # Added for preliminary calls #AC16 #38
...
MANIFEST_MAX_TOKEN_FILTER = 200000 # Filter files larger than this for context selection #AC17 #38
...
def execute_gemini_call(model: str, contents: List[types.Part], config: Optional[GenerateContentConfigType] = None, sleep_on_retry: int = SLEEP_DURATION_SECONDS, timeout: int = DEFAULT_API_TIMEOUT_SECONDS) -> str:
    ...
    while True:
        try:
            generation_config = None; tools_api = None
            ...
            elif isinstance(config, types.GenerateContentConfig): generation_config = config # Já é o tipo correto
        ...
        except concurrent.futures.TimeoutError: print(f"  API call timed out after {timeout}s. Retrying if possible...", file=sys.stderr); raise TimeoutError # Re-raise TimeoutError
        ...
        except google_genai_errors.APIError as e: print(f"  Google API Error: {e}", file=sys.stderr); raise e
...
def select_files_for_summary_batch(
    ...
    for filepath in all_candidates:
        ...
        if file_token_count > max_input_tokens: print(f"    Skipping '{filepath}' for batch: Individual file token count ({file_token_count}) exceeds max input limit ({max_input_tokens})."); processed_files.add(filepath); continue
        ...
        if potential_input_tokens <= max_input_tokens and potential_output_tokens <= max_output_tokens: batch_files.append(filepath); current_input_tokens = potential_input_tokens; current_estimated_output = potential_output_tokens
        else: continue
...
def confirm_and_modify_selection(suggested_files: List[str]) -> Optional[List[str]]:
    ...
    while True: # Loop for modification
        ...
        if not current_files: print("  (List is currently empty)")
        else:
            for i, filepath in enumerate(current_files): print(f"  {i + 1}: {filepath}")
        print("-----------------------------")
        print("Commands: 'y' (yes), 'n' (no/use default), 'a path/to/add' (add), 'r index|path' (remove), 'q' (quit)")
        choice = input("Confirm selection, modify, or use default? [Y]: ").strip().lower()
        ...
        if choice in ['y', 'yes', '']:
            ...
            if not path_to_add: print("  Error: Please provide a path after 'a'."); continue
            ...
            if '/' not in path_to_add and '.' not in path_to_add: print(f"  Warning: '{path_to_add}' doesn't look like a valid file path. Adding anyway.");
            if path_to_add in current_files: print(f"  '{path_to_add}' is already in the list."); continue
            current_files.append(path_to_add); print(f"  Added '{path_to_add}'.")
        elif choice.startswith('r '):
            item_to_remove = choice[2:].strip()
            if not item_to_remove: print("  Error: Please provide an index or path after 'r'."); continue
            removed = False
            try:
                ...
                if 0 <= index_to_remove < len(current_files):
                    ...
                    print(f"  Removed '{removed_path}' (index {index_to_remove + 1})."); removed = True
                else: print(f"  Error: Index {index_to_remove + 1} is out of bounds.");
            except ValueError:
                # If not an index, try removing by path
                if item_to_remove in current_files:
                    current_files.remove(item_to_remove)
                    print(f"  Removed '{item_to_remove}'."); removed = True
                else: print(f"  Error: Path '{item_to_remove}' not found in the list.");
            if not removed and not item_to_remove.isdigit(): # Avoid double error message if index was invalid
                 print(f"  Error: Could not remove '{item_to_remove}' (not a valid index or existing path).")
        else: print("  Invalid command. Use 'y', 'n', 'a path', 'r index', or 'r path'.")
...
def parse_arguments(all_available_tasks: List[str]) -> argparse.Namespace:
    ...
    epilog_lines.append(f"    python {script_name} resolve-ac -i 38 -a 15 -sc # Use context selector") # Updated example
    ...
    epilog_text = "\n".join(epilog_lines)
    parser = argparse.ArgumentParser(description="Interact with Google Gemini using project context and prompt templates.", epilog=epilog_text, formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    parser.add_argument("-sc", "--select-context", action="store_true", help="Enable preliminary context selection by LLM.") # AC10 #38
...
if __name__ == "__main__":
    ...
    if selected_task == "manifest-summary":
        ...
        for filepath_str, metadata in files_dict.items():
            ...
            if is_binary: continue # Skip binary files
            needs_summary = False
        ...
        summary_prompt_path = META_PROMPT_DIR / "meta-prompt-manifest-summary.txt" if args.two_stage else TEMPLATE_DIR / "prompt-manifest-summary.txt"
        ...
        base_summary_prompt_content = summary_prompt_path.read_text(encoding='utf-8') # This is either meta or direct prompt
        processed_files_in_run: Set[str] = set(); manifest_was_modified = False
        while True:
            batch_files, batch_input_tokens, batch_output_estimate = select_files_for_summary_batch(manifest_data, candidates_for_summary, processed_files_in_run, args.max_files_per_call, SUMMARY_TOKEN_LIMIT_PER_CALL, MAX_OUTPUT_TOKENS_ESTIMATE, ESTIMATED_TOKENS_PER_SUMMARY)
            ...
            contents_for_api, processed_paths_in_batch = prepare_api_content_for_summary(batch_files, base_summary_prompt_content) # Use base_summary_prompt_content
            ...
            try:
                 ...
                 for filepath, summary in parsed_summaries.items():
                     if filepath in manifest_data["files"]: # Update existing entry
                         manifest_data["files"][filepath]["summary"] = summary; processed_files_in_run.add(filepath); manifest_was_modified = True; updated_count_in_batch+=1
            ...
            processed_files_in_run.update(batch_files) # Mark all attempted files as processed for this run
        ...
        manifest_data_for_context: Optional[Dict[str, Any]] = None # Store loaded manifest if --select-context
        ...
        if args.select_context: # AC10 #38
            ...
            for path, metadata in all_manifest_files.items():
                ...
                if isinstance(token_count, int) and token_count <= MANIFEST_MAX_TOKEN_FILTER: filtered_manifest_files[path] = metadata
            ...
            if not suggested_files_from_api:
                ...
                final_selected_files = confirm_and_modify_selection(suggested_files_from_api)
                if final_selected_files is None: # User chose 'n' or 'q'
                    use_default_context = True
                    print("  User chose to use the default context or quit. Proceeding with default.")
                else:
                    use_default_context = False
                    print(f"  User confirmed using {len(final_selected_files)} selected files (after potential modifications).")
