...
def command_exists(cmd: str) -> bool:
    ...
    return (
        subprocess.run(["which", cmd], capture_output=True, text=True).returncode == 0
    )
...
def load_env_vars(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if dotenv_path.is_file():
        print(
            f"Loading environment variables from: {dotenv_path.relative_to(BASE_DIR)}"
        )
        load_dotenv(dotenv_path=dotenv_path, verbose=True)
    else:
        print(
            "No .env file found, using script defaults and CLI args.", file=sys.stderr
        )
...
def check_and_create_label(label_name: str, repo_flags: List[str], color: str) -> bool:
    ...
    if exit_code_create == 0:
        ...
        if "already exists" in stderr_create.lower():
            print(
                f"      Label '{label_name}' likely already exists (creation failed with 'already exists')."
            )
            checked_labels[label_name] = True
            return True
        else:
...
def check_and_create_milestone(
    ...
    if description:
        ...
        if exit_code_create == 0:
            ...
            if "already exists" in stderr_create.lower():
                print(
                    f"    Milestone '{title}' likely already exists (creation failed). Using title."
                )
...
def find_existing_issue(title: str, repo_flags: List[str]) -> Optional[int]:
    ...
    if exit_code != 0:
        print(
            f"    Warning: Failed to list issues (Code: {exit_code}). Stderr: {stderr.strip()}",
            file=sys.stderr,
        )
        return None
    try:
        issue_number_str = subprocess.run(
            ["jq", "-r", jq_filter],
            input=stdout,
            text=True,
            check=True,
        ...
        print(
            f"    Warning: jq processing failed (Code: {e.returncode}). Stderr: {e.stderr}",
            file=sys.stderr,
        )
        return None
    except ValueError:
        print(
            f"    Warning: Could not parse issue number from jq output: '{issue_number_str}'.",
            file=sys.stderr,
        )
        return None
...
def prepare_issue_body(
    ...
    if not template_file.is_file():
        print(
            f"    Warning: Template '{template_file.name}' not found. Trying 'test' template...",
            file=sys.stderr,
        )
        template_file = template_dir / "test_body.md"  # Fallback to test for test type
        if not template_file.is_file():
            print(
                f"    Warning: Test template not found. Trying chore template...",
                file=sys.stderr,
            )
            template_file = (
                template_dir / "chore_body.md"
            )  # Fallback to chore for chore/refactor
            if not template_file.is_file():
                print(
                    f"    Warning: Chore template not found. Trying default...",
                    file=sys.stderr,
                )
                template_file = template_dir / DEFAULT_TEMPLATE_NAME
                if not template_file.is_file():
                    print(
                        f"    Warning: Default template '{DEFAULT_TEMPLATE_NAME}' not found. Using generic body.",
                        file=sys.stderr,
                    )
                    return (
                        f"Issue created from script for title: {issue_data.get('TITLE', '[No Title]')}\n\nDetails:\n"
                        + "\n".join(
                            f"- {k}: {v}" for k, v in issue_data.items() if k != "TITLE"
                        )
                    )
    ...
    try:
        ...
        for key, value in issue_data.items():
            ...
            body = re.sub(
                ...
                lambda m: str(value) if value is not None else "",
                body,
            )
        ...
        print(
            f"    Error reading or processing template {template_file}: {e}",
            file=sys.stderr,
        )
    ...
    try:
        ...
        for i, block in enumerate(blocks):
            ...
            for line in block.strip().split("\n"):
                ...
                match = re.match(
                    r"^([A-Z_]+):\s*(.*)", line
                )  # Key must be uppercase/underscore
                if match:
                    ...
                    if current_key in ["TITLE", "PARENT_ISSUE"]:
                        issue_data[current_key] = (
                            raw_value_part  # Store raw value, no comment stripping
                        )
                    else:
                        # Original logic for other keys: strip comment after first #
                        issue_data[current_key] = raw_value_part.split("#", 1)[
                            0
                        ].rstrip()

                    current_value_lines = []  # Reset for potential multi-line
                    print(
                        f"  Parsed Key: {current_key}, Initial Value: '{issue_data[current_key]}'"
                    )
                    ...
                    if current_key not in ["TITLE", "PARENT_ISSUE"]:
                        current_value_lines.append(
                            line
                        )  # Keep original indentation/comments for now
            ...
            if "TITLE" in issue_data:
                ...
                print(
                    "  Warning: Skipping block - Missing TITLE field.", file=sys.stderr
                )
...
def create_github_issue(
    ...
    if milestone_title:
        ...
        print(
            f"    Error: Cannot assign mandatory milestone '{cli_args.milestone_title}' during creation.",
            file=sys.stderr,
        )
        return False
    ...
    if project_name_or_num:
        ...
        if project_id:
            ...
            print(
                f"    Error: Project '{project_name_or_num}' not found under '{config['project_owner']}' or '{repo_owner}'. Cannot create issue.",
                file=sys.stderr,
            )
            return False
...
def edit_github_issue(
    ...
    if milestone_title:
        ...
        print(
            f"    Error: Cannot assign mandatory milestone '{cli_args.milestone_title}' during edit.",
            file=sys.stderr,
        )
        return False
    if issue_data.get("PROJECT"):
        print(
            f"    Info: Project association ('{issue_data['PROJECT']}') is ignored during edit. Please verify/add manually if needed."
        )
...
def main(args: argparse.Namespace, config: Dict[str, Any]) -> int:
    ...
    if config["repo_target"]:
        ...
        try:
            ...
            if not repo_owner or "/" not in config["repo_target"]:
        ...
        if exit_code != 0 or not stdout.strip():
            print(
                f"Error: Failed to determine owner of current repository. Stderr: {stderr.strip()}",
                file=sys.stderr,
            )
            return 1
    ...
    if args.milestone_title:
        milestone_title = check_and_create_milestone(
            args.milestone_title, args.milestone_desc, repo_flags
        )
        if milestone_title:
            ...
            print(
                f"Error: Failed to find or create the specified milestone '{args.milestone_title}'. Aborting.",
                file=sys.stderr,
            )
            return 1
    ...
    print(
        f"Summary: {processed_count} issues processed successfully, {error_count} errors encountered."
    )
...
""",
    ...
    parser.add_argument(
        "--milestone-title",
        help="Title of the milestone to assign issues to. If it doesn't exist, it will be created if --milestone-desc is also provided.",
    )
    parser.add_argument(
        "--milestone-desc",
        help="Description for the milestone if it needs to be created.",
    )
    parser.add_argument(
        "-R",
        "--repo",
        metavar="OWNER/REPO",
        help=f"Target repository (default: current directory's repo, can be set via GH_REPO_TARGET env var). Overrides GH_REPO_TARGET if set.",
    )
    parser.add_argument(
        "--default-assignee",
        help=f"Default assignee if not specified in the plan file (default: {DEFAULT_ASSIGNEE}, can be set via DEFAULT_ASSIGNEE env var).",
    )
    parser.add_argument(
        "--default-label",
        help=f"Default label if no labels are specified (default: {DEFAULT_LABEL}, can be set via DEFAULT_LABEL env var).",
    )
    parser.add_argument(
        "--default-color",
        help=f"Default color for newly created labels (default: {DEFAULT_LABEL_COLOR}, can be set via DEFAULT_LABEL_COLOR env var).",
    )
    parser.add_argument(
        "--project-owner",
        help=f"Owner (@me or org/user) to search for projects first (default: {PROJECT_PRIMARY_OWNER}, can be set via GH_PROJECT_OWNER env var).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Simulate the process without actually creating or editing issues on GitHub.",
    )
...
if __name__ == "__main__":
    ...
    if not command_exists("jq"):
        suggest_install("jq")
        print(
            "Warning: 'jq' not found. Issue duplicate checking and project finding might fail.",
            file=sys.stderr,
        )
