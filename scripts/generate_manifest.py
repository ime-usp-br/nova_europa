...
DEFAULT_INTER_CALL_SLEEP = 0.5 # Reduzido o sleep padrão
...
genai_client: Optional[genai.Client] = None # Mudança: Usar Client diretamente
...
def get_git_versioned_status(filepath_relative: Path, verbose: bool) -> bool:
    ...
    try:
        exit_code, _, stderr = run_command(['git', 'ls-files', '--error-unmatch', filepath_relative.as_posix()], check=False, capture=True)
        return exit_code == 0
    except Exception as e:
...
def scan_project_files(verbose: bool) -> Tuple[Set[Path], Set[Path]]:
    ...
    # Use ls-files para obter uma lista base (versionados, stageados, outros não ignorados)
    if verbose: print("  Executando 'git ls-files -z -c -o --exclude-standard'...")
    ...
    # versioned_files_set será determinado depois
    return all_files_set, set()
...
def filter_files(all_files: Set[Path], default_ignores: Set[str], custom_ignores: List[str], output_filepath: Path, verbose: bool) -> Set[Path]:
    ...
    # Constrói a lista de padrões de ignore combinados
    ignore_patterns = default_ignores.copy()
    ignore_patterns.update(custom_ignores)
    try:
        # Adiciona o próprio arquivo de saída à lista de ignores
        ignore_patterns.add(output_filepath.relative_to(PROJECT_ROOT).as_posix())
    except ValueError:
        pass # output_filepath pode estar fora do root
    ...
    for file_path in all_files:
        ...
        for pattern in ignore_patterns:
            # Lógica Especial para Vendor/Context - Permitir subdirs específicos
            # Permitir vendor/uspdev/*
            if pattern == "vendor/" and any(file_path_str.startswith(str(usp_dir.relative_to(PROJECT_ROOT).as_posix())) for usp_dir in VENDOR_USPDEV_DIRS):
                continue # Não ignora se estiver dentro de um vendor/uspdev permitido
            # Permitir context_llm/common/* e context_llm/code/<latest>/*
            if pattern == "context_llm/":
                ...
                if is_common or is_latest_code:
                    continue # Não ignora se for common ou latest code
...
def get_file_type(relative_path: Path) -> str:
    ...
    if name.startswith('.env'): return 'environment_env' # Alterado para detectar qualquer .env*
    ...
    if parts[0] == 'public' and name == '.htaccess': return 'config_apache' # Especificado o nome exato
    ...
    if path_str.startswith('context_llm/code/') and len(parts) > 2:
        ...
        if file_part.startswith('github_issue_') and file_part.endswith('_details.json'): return 'context_code_issue_details'
        ...
        if any(p in path_str for p in ['/planos/', '/meta-prompts/', '/prompts/']): return 'context_code'
...
def extract_php_dependencies(file_content: str) -> List[str]:
    ...
    for base_namespace, group_content in matches_group:
        ...
        for item in items:
            if item.lower().startswith(('function ', 'const ')): continue
            parts = item.split(' as ')
...
def count_tokens_for_file(
    ...
    try:
        ...
        if verbose: print(f"      -> Token Count (AC14): Error reading file '{filepath_absolute.name}' for API call: {e}", file=sys.stderr)
        return None
    except MemoryError:
        if verbose: print(f"      -> Token Count (AC14): MemoryError reading large file '{filepath_absolute.name}' for API. Estimating.", file=sys.stderr)
        try:
...
if __name__ == "__main__":
    ...
    if not load_api_keys(args.verbose): print("Aviso: Incapaz de carregar chave(s) de API. A contagem de tokens via API será pulada (usará estimativas).")
    else: initialize_gemini(args.verbose)
    ...
    # Initialize global executor before the loop
    ...
    try:
        for file_path_relative in sorted(list(filtered_file_paths)):
            ...
            if relative_path_str.startswith(('vendor/uspdev/', 'context_llm/')): is_versioned = False # Docs laravel não deve ser sobreescrito
            if args.verbose: print(f"      -> Versioned (AC9): {is_versioned}")

            calculated_hash: Optional[str] = None
            is_env_file = file_type == 'environment_env'
            is_context_code = file_type.startswith('context_code_') # Inclui o manifest JSON copiado
            should_calculate_hash = not is_binary and not is_env_file and not is_context_code # Don't hash context code
            ...
            if should_calculate_hash:
                try:
                    ...
                    calculated_hash = hashlib.sha1(file_content_bytes).hexdigest()
                     ...
                     if args.verbose: print(f"      -> Hash (AC10/11): Error reading file content bytes: {e}", file=sys.stderr)
            elif args.verbose:
                reason_hash = "binary (AC6)" if is_binary else "env file (AC11)" if is_env_file else "context code (AC11)" if is_context_code else "unknown exclusion"
                print(f"      -> Hash (AC11): Setting to null ({reason_hash})")

            if args.verbose: print(f"      -> Hash (AC10/11): {calculated_hash or 'null'}")
            ...
            if file_type.startswith(('code_php_', 'migration_php', 'test_php_')):
                try:
                    ...
                    dependencies = []
            elif args.verbose: print(f"      -> Dependencies (AC23): Skipping (not a PHP file).")
            ...
            if should_count_tokens_or_estimate and api_executor:
                ...
                token_count_result = count_tokens_for_file(
                    api_executor,
                    file_path_absolute,
                    file_type, # Passar o tipo do arquivo
                    previous_count,
                    calculated_hash,
                    previous_hash,
                    args.verbose,
