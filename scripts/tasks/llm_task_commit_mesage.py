# -*- coding: utf-8 -*-
"""
Script para a tarefa 'commit-mesage' de interação com LLM.
Gera uma mensagem de commit Git baseada nas mudanças e histórico do projeto.
"""
...
TASK_NAME = "commit-mesage"
PROMPT_TEMPLATE_NAME = "prompt-commit-mesage.txt"
META_PROMPT_TEMPLATE_NAME = "meta-prompt-commit-mesage.txt"


def add_task_specific_args(parser: argparse.ArgumentParser):
    """Adiciona argumentos específicos da tarefa 'commit-mesage' ao parser."""
    parser.add_argument(
        "-i",
        "--issue",
        help="Número da Issue GitHub relacionada (opcional). Preenche __NUMERO_DA_ISSUE__.",
        default=None,  # Tornando opcional
    )
    # O argumento -o/--observation já é adicionado pelo get_common_arg_parser


def main_commit_mesage():
    """Função principal para a tarefa commit-mesage."""
    parser = core_args_module.get_common_arg_parser(
        description=f"Executa a tarefa '{TASK_NAME}' para gerar uma mensagem de commit."
    )
                ...
                final_selected_files_for_context = (
                    core_context.confirm_and_modify_selection(
                        ...
                        api_client.calculate_max_input_tokens(
                            core_config.GEMINI_MODEL_GENERAL_TASKS, verbose=verbose
                        )
                    )
                )
            ...
            while True:
                ...
                try:
                    prompt_final_content = api_client.execute_gemini_call(
                        core_config.GEMINI_MODEL_GENERAL_TASKS,  # Usar modelo geral para meta
                        contents_step1,
                        config=types.GenerateContentConfig(
                            tools=(
                                [
        ...
        while True:
            ...
            try:
                final_response_content = api_client.execute_gemini_call(
                    core_config.GEMINI_MODEL_GENERAL_TASKS,  # Usar modelo geral para mensagem de commit
                    contents_final,
                    config=types.GenerateContentConfig(
                        tools=(
                            [
