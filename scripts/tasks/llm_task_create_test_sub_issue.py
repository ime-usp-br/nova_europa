# -*- coding: utf-8 -*-
"""
Script para a tarefa 'create-test-sub-issue' de interação com LLM.
Gera o conteúdo para uma sub-issue de teste no GitHub.
"""
...
from scripts.llm_core import utils as core_utils # Importar utils se for usar run_command, etc.

from google.genai import types

TASK_NAME = "create-test-sub-issue"
PROMPT_TEMPLATE_NAME = "prompt-create-test-sub-issue.txt"
META_PROMPT_TEMPLATE_NAME = "meta-prompt-create-test-sub-issue.txt"


def add_task_specific_args(parser: argparse.ArgumentParser):
    """Adiciona argumentos específicos da tarefa 'create-test-sub-issue' ao parser."""
    parser.add_argument(
        "-i", "--issue", required=True, help="Número da Issue GitHub PAI (obrigatório)."
    )
    parser.add_argument(
        "-a",
        "--ac",
        help="Número do Critério de Aceite (AC) da Issue PAI para focar (opcional).",
        default="", # Default para string vazia se não fornecido
    )
    # O argumento -o/--observation já é adicionado pelo get_common_arg_parser


def main_create_test_sub_issue():
    """Função principal para a tarefa create-test-sub-issue."""
    parser = core_args_module.get_common_arg_parser(
        description=f"Executa a tarefa '{TASK_NAME}' para gerar o conteúdo de uma sub-issue de teste."
    )
    ...
    try:
        ...
        task_variables: Dict[str, str] = {
            "PARENT_ISSUE_NUMBER": args.issue, # Renomeado para consistência com o prompt
            "PARENT_AC_NUMBER": args.ac if args.ac else "", # Usa string vazia se None
        ...
        if args.two_stage:
            ...
            GEMINI_MODEL_STEP2 = core_config.GEMINI_MODEL_GENERAL_TASKS # Para esta task, modelo geral em ambas etapas
        else:
        ...
        while True:
            ...
            print(f"\n{step_name} Prompt Final + Contexto ({len(context_parts)} partes) para gerar conteúdo da sub-issue de teste...")
        ...
        if final_response_content is None:
            print("Erro: Nenhuma resposta final para a sub-issue de teste obtida.", file=sys.stderr)
            sys.exit(1)
