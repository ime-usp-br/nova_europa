# -*- coding: utf-8 -*-
"""
Script para a tarefa 'review-issue' de interação com LLM.
Revisa uma Issue GitHub existente e gera um corpo Markdown atualizado.
"""
...
TASK_NAME = "review-issue"
PROMPT_TEMPLATE_NAME = "prompt-review-issue.txt"
META_PROMPT_TEMPLATE_NAME = "meta-prompt-review-issue.txt"


def add_task_specific_args(parser: argparse.ArgumentParser):
    """Adiciona argumentos específicos da tarefa 'review-issue' ao parser."""
    parser.add_argument(
        "-i", "--issue", required=True, help="Número da Issue GitHub a ser revisada (obrigatório)."
    )
    # O argumento -o/--observation já é adicionado pelo get_common_arg_parser


def main_review_issue():
    """Função principal para a tarefa review-issue."""
    parser = core_args_module.get_common_arg_parser(
        description=f"Executa a tarefa '{TASK_NAME}' para revisar uma Issue GitHub."
    )
    ...
    try:
        ...
        if args.two_stage:
            ...
            GEMINI_MODEL_STEP2 = core_config.GEMINI_MODEL_GENERAL_TASKS # Modelo geral para texto/markdown
        else:
            ...
            GEMINI_MODEL_STEP1 = core_config.GEMINI_MODEL_GENERAL_TASKS # Não usado no fluxo direto
        ...
        while True:
            ...
            try:
                ...
                print("\n--- Resposta Final (Corpo da Issue Revisada) ---")
        ...
        if final_response_content is None:
            print("Erro: Nenhuma resposta final para a revisão da issue obtida.", file=sys.stderr)
            sys.exit(1)

        if final_response_content.strip(): # Somente salva se não estiver vazio
            save_confirm_choice, _ = io_utils.confirm_step("Confirmar salvamento desta resposta (corpo da issue revisada)?")
            if save_confirm_choice == "y":
                print("\nSalvando Resposta Final...")
                io_utils.save_llm_response(TASK_NAME, final_response_content.strip()) # Salva a resposta completa, que é o corpo da issue
            else:
