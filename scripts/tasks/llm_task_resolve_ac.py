...
from google.genai import types # Necessário para types.Part, types.GenerateContentConfig

# Constantes específicas da tarefa
TASK_NAME = "resolve-ac"
RESOLVE_AC_PROMPT_TEMPLATE_NAME = "prompt-resolve-ac.txt" # Usado no llm_interact_copy
RESOLVE_AC_META_PROMPT_TEMPLATE_NAME = "meta-prompt-resolve-ac.txt" # Usado no llm_interact_copy
...
def add_task_specific_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-i", "--issue",
        required=True,
        help="Número da Issue GitHub (obrigatório)."
    )
    parser.add_argument(
        "-a", "--ac",
        required=True,
...
def main_resolve_ac():
    ...
    try:
        ...
        if args.generate_context:
            ...
            if not core_config.CONTEXT_GENERATION_SCRIPT.is_file() or not os.access(core_config.CONTEXT_GENERATION_SCRIPT, os.X_OK):
        ...
        if args.two_stage:
            template_path_to_load = core_config.META_PROMPT_DIR / RESOLVE_AC_META_PROMPT_TEMPLATE_NAME
            print(f"\nFluxo de Duas Etapas Selecionado")
            print(f"Usando Meta-Prompt: {template_path_to_load.relative_to(core_config.PROJECT_ROOT)}")
        else:
            template_path_to_load = core_config.TEMPLATE_DIR / RESOLVE_AC_PROMPT_TEMPLATE_NAME
            print(f"\nFluxo Direto Selecionado")
            print(f"Usando Prompt: {template_path_to_load.relative_to(core_config.PROJECT_ROOT)}")

        initial_prompt_content_original = core_prompts_module.load_and_fill_template(template_path_to_load, task_variables)
        ...
        # A verificação se latest_context_dir_path é None deve ocorrer ANTES de seu uso
        # para o carregamento padrão. Se --select-context for usado e falhar, ou for recusado,
        # e o carregamento padrão for ativado, precisamos garantir que latest_context_dir_path exista.
        ...
        if args.select_context:
            ...
            if not manifest_data_for_context_selection or "files" not in manifest_data_for_context_selection:
                ...
                sys.exit(1)
            print(f"  Manifesto carregado: {latest_manifest_path.relative_to(core_config.PROJECT_ROOT)}")
            ...
            if not context_selector_prompt_path:
                # find_context_selector_prompt já imprime o erro
                sys.exit(1) 
            ...
            if not selector_prompt_content:
                ...
                sys.exit(1)
            print(f"  Usando Prompt Seletor: {context_selector_prompt_path.relative_to(core_config.PROJECT_ROOT)}")
            ...
            if verbose: print(f"    Excluídos {excluded_count} arquivos do manifesto para a API seletora devido ao limite de tokens.")
            ...
            try:
                ...
                if verbose: print(f"    Payload para API seletora (início): {preliminary_api_input_content[:200]}...")
            ...
            try:
                print(f"  Chamando API preliminar ({core_config.GEMINI_MODEL_FLASH}) para seleção de contexto...")
                response_prelim_str = api_client.execute_gemini_call(
                    ...
                    config=types.GenerateContentConfig(tools=([types.Tool(google_search_retrieval=types.GoogleSearchRetrieval())] if args.web_search else [])), # Ferramentas baseadas em args.web_search
                    verbose=verbose
                )
        ...
        if final_selected_files_for_context is not None and not load_default_context_after_selection_failure:
            ...
            if not latest_context_dir_path:
                print("Erro fatal: Nenhum diretório de contexto encontrado para carregamento padrão. Execute generate_context.py.", file=sys.stderr)
                sys.exit(1)
            context_parts = core_context.prepare_context_parts(
        ...
        if not context_parts and verbose: # Adicionado verbose para não poluir tanto
            print("Aviso: Nenhuma parte de contexto carregada. A LLM pode não ter informações suficientes.", file=sys.stderr)
            ...
            while True:
                ...
                try:
                    prompt_final_content = api_client.execute_gemini_call(
                        core_config.GEMINI_MODEL_RESOLVE, # Modelo da tarefa
                        contents_step1,
                    ...
                    if args.yes:
                        ...
                        user_choice_step1, observation_step1 = io_utils.confirm_step("Usar este prompt gerado para a Etapa 2?")
        ...
        while True:
            ...
            try:
                ...
                if args.yes:
                    ...
                    user_choice_final, observation_final = io_utils.confirm_step("Prosseguir com esta resposta final?")
