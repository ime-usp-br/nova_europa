...
# NOVO (Issue #69): Permite execução seletiva de estágios de coleta.
#
# Dependencies Base:
#   - Python 3.10+
#   - Git
...
# Original TOTAL_STEPS was 19 including manifest generator and final manifest.md
# Collection stages are 15.
# Steps for manifest.json and manifest.md generation will run after collection.
NUM_FINALIZATION_STEPS = 3 # invoke_manifest_generator, copy_latest_manifest_json, generate_manifest_md
...
def collect_git_info(
    ...
    args = cli_args # Para clareza de que estamos usando os args passados
    run_command(
    ...
    if exit_code == 0 and stdout:
        ...
        limit = args.tag_limit if args else DEFAULT_GIT_TAG_LIMIT
        tags_file.write_text("\n".join(lines[:limit]) + "\n", encoding="utf-8")
        print(f"    {min(len(lines), limit)} tags recentes salvas.")
    elif exit_code == 0:
...
def collect_gh_info(
    ...
    limit_runs = args.run_limit if args else DEFAULT_GH_RUN_LIST_LIMIT
    limit_prs = args.pr_limit if args else DEFAULT_GH_PR_LIST_LIMIT
    limit_releases = args.release_limit if args else DEFAULT_GH_RELEASE_LIST_LIMIT

    run_command(["gh", "run", "list", "--limit", str(limit_runs)], output_dir / "gh_run_list.txt")
    run_command(["gh", "workflow", "list"], output_dir / "gh_workflow_list.txt")
    run_command(["gh", "pr", "list", "--state", "all", "--limit", str(limit_prs)], output_dir / "gh_pr_list.txt")
    run_command(["gh", "release", "list", "--limit", str(limit_releases)], output_dir / "gh_release_list.txt")
...
def collect_gh_project_info(
    ...
    project_num = args.gh_project_number if args else DEFAULT_GH_PROJECT_NUMBER
    project_owner = args.gh_project_owner if args else DEFAULT_GH_PROJECT_OWNER
    status_field = args.gh_project_status_field if args else DEFAULT_GH_PROJECT_STATUS_FIELD_NAME
...
def collect_structure_info(
    ...
    args = cli_args
    tree_depth = args.tree_depth if args else DEFAULT_TREE_DEPTH
    tree_file = output_dir / f"project_tree_L{tree_depth}.txt"
    cloc_file = output_dir / "project_cloc.txt"
    if command_exists("tree"):
        run_command(["tree", "-L", str(tree_depth), "-a", "-I", TREE_IGNORE_PATTERN], tree_file)
    else:
...
def collect_github_issue_details(
    ...
    limit = args.issue_limit if args else DEFAULT_GH_ISSUE_LIST_LIMIT
    print(f"  Listando e filtrando issues (limite: {limit}, excluindo 'Closed as not planned')...")
    list_cmd = ["gh", "issue", "list", "--state", "all", "--limit", str(limit), "--json", "number,stateReason"]
    try:
        ...
        if command_exists("jq"):
            ...
            issue_numbers = []
            raw_issues = json.loads(list_process.stdout) # Assuming gh returns JSON even for non --json list
...
def _run_phpstan_check_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
    """Wrapper for PHPStan check as a stage."""
...
def _run_pint_check_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
    """Wrapper for Pint check as a stage."""
...
def _run_dusk_suite_stage(
    ...
    """Wrapper for the entire Dusk suite (run_dusk_tests and create_dusk_note) as a single stage."""
    # Effectively, this stage has two sub-steps, but is treated as one for --stages
    run_dusk_tests(output_dir, step_num, total_steps) # Passa os mesmos step_num/total_steps
    create_dusk_note(output_dir, step_num, total_steps) # Idem

def run_dusk_tests(output_dir: Path, step_num: int, total_steps: int): # Interno do _run_dusk_suite_stage
...
def create_dusk_note(output_dir: Path, step_num: int, total_steps: int): # Interno do _run_dusk_suite_stage
...
def run_python_tests(
    ...
    pytest_output_file = output_dir / "pytest_results.txt"
    if not PYTHON_CMD:
...
# Função para executar PHPStan (parte de run_quality_checks)
def _run_phpstan_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
    ...
    _run_phpstan_check_stage(output_dir, step_num, total_steps, cli_args)

# Função para executar Pint (parte de run_quality_checks)
def _run_pint_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
    print(f"[{step_num}/{total_steps}] Verificando estilo de código com Pint...")
    _run_pint_check_stage(output_dir, step_num, total_steps, cli_args)

# Função para executar Dusk (run_dusk_tests e create_dusk_note)
def _run_dusk_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
    print(f"[{step_num}/{total_steps}] Executando suíte de testes Dusk completa...")
    _run_dusk_suite_stage(output_dir, step_num, total_steps, cli_args)
...
STAGES_CONFIG: Dict[str, Dict[str, Any]] = {
    ...
    "dusk": {"func": _run_dusk_stage, "description": "Dusk browser test results and info", "needs_cli_args": False},
    "pytest": {"func": run_python_tests, "description": "Pytest (Python tests) results", "needs_cli_args": False},
}


# --- Função Principal Orchestrator (Refatorada para AC #69) ---
def run_all_collections(output_dir: Path, timestamp: str, args: argparse.Namespace):
    # Determina quais estágios rodar
    stages_to_run_names = args.stages if args.stages else list(STAGES_CONFIG.keys())
    
    # Filtra STAGES_CONFIG para incluir apenas os estágios a serem executados, mantendo a ordem original
    active_stages_ordered_dict: Dict[str, Dict[str, Any]] = {
        name: STAGES_CONFIG[name] for name in STAGES_CONFIG if name in stages_to_run_names
    }
    
    effective_total_collection_steps = len(active_stages_ordered_dict)
    current_collection_step = 0

    print(f"\n--- Iniciando Coleta Seletiva de Contexto ({effective_total_collection_steps} estágios selecionados) ---")

    for stage_name, stage_config in active_stages_ordered_dict.items():
        current_collection_step += 1
        print(f"\n[{current_collection_step}/{effective_total_collection_steps}] Executando estágio: {stage_name} ({stage_config['description']})...")
        
        func_to_call: Callable = stage_config["func"]
        
        try:
            if stage_config["needs_cli_args"]:
                func_to_call(output_dir, current_collection_step, effective_total_collection_steps, args)
            else:
                func_to_call(output_dir, current_collection_step, effective_total_collection_steps)
        except Exception as e:
            print(f"    ERRO INESPERADO no estágio '{stage_name}': {e}", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)
            # Decide se continua ou para. Por enquanto, continua.
            # Futuramente, pode ter uma flag --stop-on-stage-failure
    
    print("\n--- Coleta Seletiva de Contexto Concluída ---")

    # Etapas de finalização que sempre rodam
    final_step_counter = effective_total_collection_steps
    
    if MANIFEST_GENERATOR_SCRIPT.is_file() and os.access(MANIFEST_GENERATOR_SCRIPT, os.X_OK):
        final_step_counter +=1
        invoke_manifest_generator(output_dir, timestamp, args) # Já loga seu próprio step
    
    final_step_counter += 1
    copy_latest_manifest_json(output_dir, final_step_counter, final_step_counter + 1)
    
    final_step_counter += 1
    generate_manifest_md(output_dir, timestamp) # Já loga seu próprio step como 'Final'
...
""",
    ...
    # AC1 Issue #69: Adiciona o argumento --stages
    parser.add_argument(
        "-s", "--stages",
        nargs='*',  # Aceita zero ou mais argumentos para --stages
        choices=list(STAGES_CONFIG.keys()), # Valida contra os nomes de estágio definidos
        metavar='STAGE_NAME',
        ...
        default=None # Será tratado para rodar todos se None
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    print(f"Versão do Script: {Path(__file__).name} v0.1.0 ( Selective Stages Implemented - Issue #69 )")
    ...
    # A refatoração de run_all_collections para usar STAGES_CONFIG e args.stages é crucial aqui
    ...
    print(f"Consulte '{timestamp_dir_path.relative_to(BASE_DIR)}/manifest.md' para um resumo.")
    print("-" * 50)
    sys.exit(0)
