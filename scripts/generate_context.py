...
# ==============================================================================
# generate_context.py (v1.7 - Copies latest manifest JSON)
#
...
def run_quality_checks(output_dir: Path, step_num: int, total_steps: int):
    ...
    print(f"[{step_num+1}/{total_steps}] Verificando estilo de código com Pint...") # Ajuste o número total se necessário
...
def run_dusk_tests(output_dir: Path, step_num: int, total_steps: int):
    ...
    if not (BASE_DIR / 'artisan').is_file() or not command_exists('php'):
        ...
        print(f"  {warning_msg.strip()}", file=sys.stderr)
        return
    if not dusk_env_file.is_file(): print(f"  AVISO: Arquivo de ambiente Dusk '{dusk_env_file.name}' não encontrado. Os testes Dusk podem falhar.", file=sys.stderr)
    ...
    try:
        ...
        if exit_code != 0: print(f"  AVISO: Testes Dusk falharam (Código: {exit_code}). Veja {DUSK_OUTPUT_FILE_NAME}.", file=sys.stderr)
        else: print(f"  Tentativa de execução Dusk concluída (Código: {exit_code}). Verifique {DUSK_OUTPUT_FILE_NAME} para saída.")
...
def generate_manifest(output_dir: Path, timestamp: str):
    ...
    try:
        with open(manifest_file, 'w', encoding='utf-8') as f:
            f.write(f"# Manifesto de Contexto - Gerado por {Path(__file__).name} v1.7\n") # Versão Atualizada
    ...
    except Exception as e: print(f"  ERRO: Falha ao gerar o manifesto (manifest.md): {e}", file=sys.stderr); traceback.print_exc(file=sys.stderr)
...
def run_all_collections(output_dir: Path, timestamp: str, args: argparse.Namespace):
    ...
    step += 1; copy_project_files(output_dir, step, TOTAL_STEPS)
    step += 1; collect_github_issue_details(output_dir, args, step, TOTAL_STEPS)
    # Executa quality checks (phpstan, pint)
    step += 1; run_quality_checks(output_dir, step, TOTAL_STEPS) # Agora ocupa a etapa 11 e 12 internamente
    step += 1 # Ajusta o contador para a próxima etapa (13)
    step += 1; run_tests(output_dir, step, TOTAL_STEPS) # PHPUnit (Etapa 13)
    step += 1; run_dusk_tests(output_dir, step, TOTAL_STEPS) # Dusk (Etapa 14)
    step += 1; create_dusk_note(output_dir, step, TOTAL_STEPS) # Nota Dusk (Etapa 15)

    # --- AC22 #32: Invocar generate_manifest.py ---
    # Assume-se que esta etapa aconteça aqui, ANTES de tentar copiar o JSON.
    step += 1
    print(f"[{step}/{TOTAL_STEPS}] Invocando generate_manifest.py para criar/atualizar manifesto JSON...")
    if MANIFEST_GENERATOR_SCRIPT.is_file() and os.access(MANIFEST_GENERATOR_SCRIPT, os.X_OK):
        ...
        manifest_json_output_file = output_dir / "generate_manifest_py_output.log" # Log da execução
    ...
    step += 1; copy_latest_manifest_json(output_dir, step, TOTAL_STEPS) # Etapa 17
...
if __name__ == "__main__":
    ...
    print(f"Versão do Script: {Path(__file__).name} v1.7 (Copies Manifest JSON)") # Versão atualizada
