...
TIMESTAMP_DIR_REGEX = r"^\d{8}_\d{6}$" # Regex para validar nomes de diretório de timestamp
...
def find_second_latest_context_dir(output_base_path: Path) -> Optional[Path]:
    ...
    if not output_base_path.is_dir():
        print(f"  Aviso (AC5.1): Diretório base de output '{output_base_path.name}' não encontrado. Nenhum fallback possível.")
        return None
    
    valid_context_dirs = [
        d for d in output_base_path.iterdir()
        if d.is_dir() and re.match(TIMESTAMP_DIR_REGEX, d.name)
    ]
    
    if len(valid_context_dirs) < 2:
        print(f"  Aviso (AC5.1): Menos de dois diretórios de contexto anteriores encontrados em '{output_base_path.name}'. Nenhum fallback possível.")
        return None
        
    sorted_dirs = sorted(valid_context_dirs, key=lambda p: p.name, reverse=True)
    second_latest_dir = sorted_dirs[1] 
    print(f"  Info (AC5.1): Diretório de contexto anterior para fallback: {second_latest_dir.name}")
    return second_latest_dir

# --- Collection Functions ---
def collect_env_info(
    output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None
):
    print(
...
def collect_gh_info(
    ...
    if not command_exists("gh"):
        ...
        for fname in [
            "gh_run_list.txt", "gh_workflow_list.txt", "gh_pr_list.txt",
            "gh_release_list.txt", "gh_secret_list.txt", "gh_variable_list.txt",
            "gh_repo_view.txt", "gh_ruleset_list.txt",
            "gh_codescanning_alert_list.txt", "gh_dependabot_alert_list.txt",
        ]:
            write_warning_to_file(output_dir / fname, warning_msg)
        return

    run_command(["gh", "run", "list", "--limit", str(cli_args.run_limit)], output_dir / "gh_run_list.txt")
    run_command(["gh", "workflow", "list"], output_dir / "gh_workflow_list.txt")
    run_command(["gh", "pr", "list", "--state", "all", "--limit", str(cli_args.pr_limit)], output_dir / "gh_pr_list.txt")
    run_command(["gh", "release", "list", "--limit", str(cli_args.release_limit)], output_dir / "gh_release_list.txt")
...
def collect_gh_project_info(
    ...
    print(f"  Coletando itens do Projeto #{cli_args.gh_project_number} (Owner: {cli_args.gh_project_owner})...")
    cmd_list = ["gh", "project", "item-list", str(cli_args.gh_project_number), "--owner", cli_args.gh_project_owner, "--format", "json"]
    ...
    if not command_exists("jq"):
        ...
        print(f"  {warning_msg_jq.splitlines()[0]} Não foi possível gerar o resumo {summary_file.name}.", file=sys.stderr)
    ...
    print(f"  Gerando resumo de status dos itens (usando jq e campo '{cli_args.gh_project_status_field}')...")
    jq_filter = f"""
    [ .items[] |
        {{
            type: .type,
    ...
    try:
        ...
        print(f"  ERRO: {error_msg_jq_run} Veja o erro completo no arquivo de log.", file=sys.stderr)
        write_warning_to_file(summary_file, f"ERRO: {error_msg_jq_run}\nStderr:\n{e.stderr}\nStdout:\n{e.stdout}\n")
    except FileNotFoundError:
        warning_msg_jq = suggest_install("jq")
        print(f"  {warning_msg_jq.splitlines()[0]} Não foi possível gerar o resumo {summary_file.name}.", file=sys.stderr)
        write_warning_to_file(summary_file, warning_msg_jq)
        ...
        traceback.print_exc(file=sys.stderr)
        write_warning_to_file(summary_file, f"ERRO: {error_msg_jq_unexpected}\n{traceback.format_exc()}\n")
...
def collect_structure_info(
    ...
    if command_exists("tree"):
        run_command(["tree", "-L", str(cli_args.tree_depth), "-a", "-I", TREE_IGNORE_PATTERN], tree_file)
    else:
        write_warning_to_file(tree_file, suggest_install("tree"))
    if command_exists("cloc"):
        run_command(["cloc", ".", "--fullpath", f"--not-match-d={CLOC_EXCLUDE_REGEX}"], cloc_file)
    else:
...
def copy_project_files(
    ...
    dirs_to_copy = {"planos": BASE_DIR / "planos", "meta-prompts": BASE_DIR / "templates" / "meta-prompts", "prompts": BASE_DIR / "templates" / "prompts", "context_selectors": BASE_DIR / "templates" / "context_selectors"}
            ...
            for src_file in src_dir.glob("*.txt"): # Apenas .txt
                if src_file.is_file():
                    try:
                        ...
                        print(f"    Erro ao copiar {src_file.name}: {e}", file=sys.stderr)
...
def collect_github_issue_details(
    ...
    print(f"  Listando e filtrando issues (limite: {cli_args.issue_limit}, excluindo 'Closed as not planned')...")
    list_cmd = ["gh", "issue", "list", "--state", "all", "--limit", str(cli_args.issue_limit), "--json", "number,stateReason"]
    try:
        ...
        if command_exists("jq"):
            ...
            for issue_data in raw_issues:
                if isinstance(issue_data, dict) and issue_data.get("stateReason") != "NOT_PLANNED":
    ...
    if not issue_numbers:
        ...
        write_warning_to_file(issues_dir / "no_issues_found.json", '{ "message": "Nenhuma issue encontrada (após filtrar)." }\n')
        return
    ...
    for issue_number in issue_numbers:
        ...
        if exit_code == 0:
            ...
            print(f"    AVISO: Falha ao coletar detalhes da Issue #{issue_number} (Código: {exit_code}).", file=sys.stderr)
        time.sleep(0.2)
    print(f"  Coleta de {downloaded_count}/{len(issue_numbers)} issues concluída.")

def _run_phpstan_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
    phpstan_file = output_dir / "phpstan_analysis.txt"
    if PHPSTAN_BIN.is_file() and os.access(PHPSTAN_BIN, os.X_OK):
        run_command([str(PHPSTAN_BIN), "analyse", "--no-progress", "--memory-limit=2G"], phpstan_file)
    else:
...
def _run_pint_stage(output_dir: Path, step_num: int, total_steps: int, cli_args: Optional[argparse.Namespace] = None):
...
def run_tests( # PHPUnit
    ...
    if ARTISAN_FILE.is_file() and command_exists("php"):
        exit_code, _, stderr = run_command(ARTISAN_CMD + ["test", "--env=testing"], phpunit_output_file, check=False)
        try:
            ...
            if exit_code != 0:
                print(f"  AVISO: Testes PHPUnit falharam (Código: {exit_code}). Veja {phpunit_output_file.name}.", file=sys.stderr)
            else:
                print(f"  Testes PHPUnit executados com sucesso (Código: 0).")
        except Exception as e:
            print(f"  ERRO: Não foi possível anexar o código de saída ao arquivo {phpunit_output_file.name}: {e}", file=sys.stderr)
    else:
...
def _run_dusk_tests_internal(output_dir: Path, step_num: int, total_steps: int):
    ...
    if not dusk_env_file.is_file():
        print(f"    AVISO: Arquivo de ambiente Dusk '{dusk_env_file.name}' não encontrado. Os testes Dusk podem falhar.", file=sys.stderr)
    ...
    try:
        ...
        if exit_code != 0:
            print(f"    AVISO: Testes Dusk falharam (Código: {exit_code}). Veja {DUSK_OUTPUT_FILE_NAME}.", file=sys.stderr)
        else:
...
STAGES_CONFIG: Dict[str, Dict[str, Any]] = {
    "env": {"func": collect_env_info, "description": "Environment information (OS, PHP, Node)", "needs_cli_args": False,
            "outputs": ["env_uname.txt", "env_distro_info.txt", "env_php_version.txt", "env_php_modules.txt", "env_node_version.txt", "env_npm_version.txt"]},
    "python_env": {"func": collect_python_env_info, "description": "Python environment details", "needs_cli_args": False,
                   "outputs": ["env_python_version.txt", "env_python_which.txt", "env_pip_version.txt", "env_pip_freeze.txt", "env_python_venv_status.txt", "env_python_pkgfile_requirements.txt", "env_python_pkgfile_requirements-dev.txt", "env_python_pkgfile_Pipfile", "env_python_pkgfile_pyproject.toml"]},
    "git": {"func": collect_git_info, "description": "Git repository information (log, diffs, status)", "needs_cli_args": True,
            "outputs": ["git_log.txt", "git_diff_empty_tree_to_head.txt", "git_diff_cached.txt", "git_diff_unstaged.txt", "git_status.txt", "git_ls_files.txt", "git_recent_tags.txt"]},
    "github": {"func": collect_gh_info, "description": "GitHub general info (repo, actions, PRs, releases)", "needs_cli_args": True,
               "outputs": ["gh_run_list.txt", "gh_workflow_list.txt", "gh_pr_list.txt", "gh_release_list.txt", "gh_secret_list.txt", "gh_variable_list.txt", "gh_repo_view.txt", "gh_ruleset_list.txt", "gh_codescanning_alert_list.txt", "gh_dependabot_alert_list.txt"]},
    "github_project": {"func": collect_gh_project_info, "description": "GitHub Project items status", "needs_cli_args": True,
                       "outputs": ["gh_project_items_status.json", "gh_project_items_summary.json"]},
    "artisan": {"func": collect_artisan_info, "description": "Laravel Artisan command outputs", "needs_cli_args": False,
                "outputs": ["artisan_route_list.json", "artisan_route_list.txt", "artisan_about.json", "artisan_about.txt", "artisan_db_show.json", "artisan_db_show.txt", "artisan_channel_list.txt", "artisan_event_list.txt", "artisan_permission_show.txt", "artisan_queue_failed.txt", "artisan_schedule_list.txt", "artisan_env.txt", "artisan_migrate_status.txt", "artisan_config_show_app.txt", "artisan_config_show_database.txt"]},
    "dependencies": {"func": collect_dependency_info, "description": "Project dependencies (Composer, NPM, Pip)", "needs_cli_args": False,
                     "outputs": ["composer_show.txt", "npm_list_depth0.txt"]}, 
    "structure": {"func": collect_structure_info, "description": "Project structure (tree, cloc)", "needs_cli_args": True,
                  "outputs": ["project_tree_L*.txt", "project_cloc.txt"]},
    "github_issues": {"func": collect_github_issue_details, "description": "Details of specific GitHub issues", "needs_cli_args": True,
                      "outputs": ["github_issue_*_details.json", "github_issues_skipped.log", "github_issues_error.log", "no_issues_found.json"]},
    "project_files_copy": {"func": copy_project_files, "description": "Copies plans and meta-prompts", "needs_cli_args": False,
                           "outputs": ["*.txt"]}, 
    "phpstan": {"func": _run_phpstan_stage, "description": "PHPStan static analysis results", "needs_cli_args": False,
                "outputs": ["phpstan_analysis.txt"]},
    "pint": {"func": _run_pint_stage, "description": "Pint code style check results", "needs_cli_args": False,
             "outputs": ["pint_test_results.txt"]},
    "phpunit": {"func": run_tests, "description": "PHPUnit test results", "needs_cli_args": False,
                "outputs": [PHPUNIT_OUTPUT_FILE_NAME]},
    "dusk": {"func": _run_dusk_suite_stage, "description": "Dusk browser test results and info", "needs_cli_args": False,
             "outputs": [DUSK_OUTPUT_FILE_NAME, DUSK_INFO_FILE_NAME]},
    "pytest": {"func": run_python_tests, "description": "Pytest (Python tests) results", "needs_cli_args": False,
...
def run_all_collections(output_dir_current_run: Path, timestamp: str, args: argparse.Namespace):
    stages_to_run_names_from_args = args.stages if args.stages is not None else list(STAGES_CONFIG.keys())
    
    active_stages_config: Dict[str, Dict[str, Any]] = {
        name: STAGES_CONFIG[name] for name in STAGES_CONFIG if name in stages_to_run_names_from_args
    }
    skipped_stages_names: Set[str] = set(STAGES_CONFIG.keys()) - set(active_stages_config.keys())
    ...
    if args.verbose:
        ...
        print(f"  Estágios a serem executados ({len(active_stages_config)}): {', '.join(active_stages_config.keys())}")
        if skipped_stages_names:
            print(f"  Estágios pulados ({len(skipped_stages_names)}) (tentará fallback): {', '.join(skipped_stages_names)}")
    ...
    if skipped_stages_names and args.stages is not None: 
        ...
        if previous_context_dir_path:
            print(f"  [Fallback AC5.1] Usando diretório anterior para fallback: {previous_context_dir_path.name}")
            for skipped_stage_name in sorted(list(skipped_stages_names)):
                ...
                if not stage_config_details or "outputs" not in stage_config_details:
                    if args.verbose:
                        print(f"    Aviso: Estágio pulado '{skipped_stage_name}' não tem 'outputs' definidos. Não é possível copiar.", file=sys.stderr)
                    continue
                
                print(f"    [Fallback AC5.2] Copiando arquivos do estágio pulado '{skipped_stage_name}' de '{previous_context_dir_path.name}' para '{output_dir_current_run.name}'...")
                ...
                for output_pattern_or_name in stage_config_details["outputs"]:
                    ...
                    if "*" in output_pattern_or_name or "?" in output_pattern_or_name or "[" in output_pattern_or_name:
                        # Lógica para padrões glob
                        for f_prev in previous_context_dir_path.iterdir():
                            if f_prev.is_file() and fnmatch.fnmatch(f_prev.name, output_pattern_or_name):
                                files_to_copy_from_previous.append(f_prev)
                                found_pattern_in_previous = True
                        if not found_pattern_in_previous and args.verbose:
                             print(f"      Nenhum arquivo correspondente ao padrão '{output_pattern_or_name}' encontrado para o estágio '{skipped_stage_name}' no diretório anterior.")
                    else: 
                        exact_file_in_previous = previous_context_dir_path / output_pattern_or_name
                        if exact_file_in_previous.is_file():
                            ...
                            print(msg) 
                            errors_in_this_stage_copy.append(f"{output_pattern_or_name} (not found in previous context)")
                            
                    for source_file in files_to_copy_from_previous:
                        dest_file = output_dir_current_run / source_file.name
                        try:
                            shutil.copy2(source_file, dest_file)
                            if args.verbose: print(f"      Copiado (fallback): {source_file.name} -> {dest_file.name}")
                            copied_any_for_this_stage = True
                        except Exception as e_copy:
                            err_msg = f"      ERRO ao copiar (fallback) {source_file.name}: {e_copy}"
                            print(err_msg, file=sys.stderr)
                            errors_in_this_stage_copy.append(f"{source_file.name} (copy error: {e_copy})")
                ...
                if copied_any_for_this_stage and not errors_in_this_stage_copy:
                    ...
                    failed_copy_stages_details[skipped_stage_name] = errors_in_this_stage_copy
                elif args.verbose and not copied_any_for_this_stage: 
                    print(f"    Nenhum arquivo relevante encontrado ou copiado para o estágio pulado '{skipped_stage_name}' de '{previous_context_dir_path.name}'.")
            ...
            print(msg_ac6)
            print(f"               Apenas os arquivos dos estágios executados ({', '.join(sorted(list(active_stages_config.keys())))}) estarão presentes.")
    ...
    for stage_name, stage_config_data in active_stages_config.items():
        ...
        try:
            if stage_config_data["needs_cli_args"]:
                func_to_call(output_dir_current_run, current_collection_step, effective_total_collection_steps, args)
            else:
                func_to_call(output_dir_current_run, current_collection_step, effective_total_collection_steps)
            executed_stage_names.append(stage_name) 
        except Exception as e_stage:
            print(f"    ERRO INESPERADO no estágio '{stage_name}': {e_stage}", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)
    ...
    if args.verbose or True: 
        ...
        for stage_name_ordered in STAGES_CONFIG.keys(): 
            if stage_name_ordered in executed_stage_names:
                ...
                print(f"  - Estágio '{stage_name_ordered}': COPIADO (fallback do anterior)")
            elif stage_name_ordered in failed_copy_stages_details:
                details = "; ".join(failed_copy_stages_details[stage_name_ordered])
                print(f"  - Estágio '{stage_name_ordered}': TENTATIVA DE CÓPIA FALHOU/INCOMPLETA (Detalhes: {details})")
            elif stage_name_ordered in skipped_no_fallback_stages:
                print(f"  - Estágio '{stage_name_ordered}': PULADO (sem fallback, diretório anterior não encontrado)")
            elif stage_name_ordered in skipped_stages_names: 
                print(f"  - Estágio '{stage_name_ordered}': PULADO (não selecionado, fallback não aplicável ou arquivos não encontrados no anterior)")
            else: 
                 if stage_name_ordered not in active_stages_config: 
                    print(f"  - Estágio '{stage_name_ordered}': NÃO PROCESSADO (não selecionado, não tentou fallback)")
                 else: 
                    print(f"  - Estágio '{stage_name_ordered}': FALHOU NA EXECUÇÃO (erro não capturado explicitamente no log de status)")
        print("------------------------------------")

    final_step_start_num = effective_total_collection_steps + 1
    if MANIFEST_GENERATOR_SCRIPT.is_file() and os.access(MANIFEST_GENERATOR_SCRIPT, os.X_OK):
        invoke_manifest_generator(output_dir_current_run, timestamp, args) 
    
    copy_latest_manifest_json(output_dir_current_run, final_step_start_num + 1, final_step_start_num + NUM_FINALIZATION_STEPS -1) 
    generate_manifest_md(output_dir_current_run, timestamp)


def invoke_manifest_generator(output_dir: Path, timestamp: str, args: argparse.Namespace):
    print(f"[{'Final'}] Invocando generate_manifest.py para criar/atualizar manifesto JSON...")
    if MANIFEST_GENERATOR_SCRIPT.is_file() and os.access(MANIFEST_GENERATOR_SCRIPT, os.X_OK):
        cmd_manifest = [sys.executable, str(MANIFEST_GENERATOR_SCRIPT)]
        if args.verbose: cmd_manifest.append("-v")
        manifest_json_output_file = output_dir / "generate_manifest_py_output.log"
        exit_code_manifest, _, stderr_manifest = run_command(cmd_manifest, manifest_json_output_file, check=False)
        if exit_code_manifest != 0:
            print(f"  AVISO: A execução de generate_manifest.py falhou (Código: {exit_code_manifest}). O manifesto JSON pode não ter sido gerado/atualizado.", file=sys.stderr)
    else:
        print(f"  AVISO: Script generate_manifest.py não encontrado ou não executável em '{MANIFEST_GENERATOR_SCRIPT}'. Pulando geração do manifesto JSON.", file=sys.stderr)


def find_latest_manifest_json(manifest_data_dir: Path) -> Optional[Path]:
    if not manifest_data_dir.is_dir():
        print(f"  Aviso: Diretório de dados do manifesto '{manifest_data_dir.relative_to(BASE_DIR)}' não encontrado.", file=sys.stderr)
        return None
    ...
    if not manifest_files:
        print(f"  Aviso: Nenhum arquivo de manifesto JSON encontrado em '{manifest_data_dir.relative_to(BASE_DIR)}'.", file=sys.stderr)
        return None
    latest_manifest_path = sorted(manifest_files, reverse=True)[0]
    print(f"  Encontrado manifesto JSON mais recente: '{latest_manifest_path.relative_to(BASE_DIR)}'")
    return latest_manifest_path


def copy_latest_manifest_json(output_dir: Path, step_num: int, total_steps: int):
    print(f"[{step_num}/{total_steps}] Copiando manifesto JSON mais recente para o diretório de contexto...")
    ...
    if latest_manifest_path:
        try:
            ...
            print(f"  Manifesto '{latest_manifest_path.name}' copiado para '{dest_path.relative_to(BASE_DIR)}'.")
        except Exception as e:
            print(f"  ERRO: Falha ao copiar o manifesto JSON '{latest_manifest_path.name}': {e}", file=sys.stderr)
    else:
...
def generate_manifest_md(output_dir: Path, timestamp: str):
    ...
    try:
        with open(manifest_file, "w", encoding="utf-8") as f:
            f.write(f"# Manifesto de Contexto - Gerado por {Path(__file__).name} v0.1.0\n")
            ...
            f.write("- O arquivo `YYYYMMDD_HHMMSS_manifest.json` (se presente) contém o manifesto JSON detalhado gerado por `generate_manifest.py`.\n")
...
""",
    ...
    parser.add_argument(
        "-o", "--output-dir", type=Path, default=DEFAULT_OUTPUT_BASE_DIR,
...
if __name__ == "__main__":
    ...
    print(f"Versão do Script: {Path(__file__).name} v0.1.0 (AC #69.5 Fallback Implemented)")
    ...
    try:
        ...
        print(f"Diretório de saída para esta execução: {current_run_output_dir.resolve()}")
    except OSError as e:
        print(f"ERRO FATAL: Não foi possível criar o diretório de saída '{current_run_output_dir}'. Erro: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Consulte '{current_run_output_dir.relative_to(BASE_DIR)}/manifest.md' para um resumo.")
    print("-" * 50)
