...
def run_command(
    ...
    try:
        ...
        return (
            ...
            e.stdout or "",
            e.stderr
            or f"TimeoutExpired: Command '{cmd_str}' timed out after {timeout} seconds",
        )
...
def start_background_process(
    cmd_list: List[str], log_file: Path, env: Optional[Dict[str, str]] = None
    ...
    try:
        ...
        log(
            f"Erro: Comando '{cmd_list[0]}' não encontrado ao tentar iniciar em background."
        )
        if log_handle and not log_handle.closed:
            log_handle.close()  # Fecha se ocorreu erro
        return None
    except Exception as e:
...
def check_process_running(p: Optional[subprocess.Popen], name: str) -> bool:
    ...
    if p.poll() is None:
        ...
        log(
            f"  Erro: {name} (PID: {p.pid}) terminou inesperadamente com código {p.returncode}. Verifique '{name}.log'."
        )
...
def get_chromedriver_path() -> Optional[Path]:
    ...
    if system == "Linux":
        ...
        log(
            f"  Aviso: Sistema operacional '{system}' não suportado para detecção automática do ChromeDriver."
        )
        return None
    ...
    for p in processes_to_stop:
        if p.poll() is None:  # Só tenta parar se ainda estiver rodando
            ...
            try:
                ...
                log(
                    f"    Processo {pid_info} não encerrou com terminate/SIGTERM. Forçando kill..."
                )
                try:
                    ...
                    log(
                        f"      Erro ao forçar encerramento do processo {pid_info}: {kill_e}"
                    )
            except Exception as e:
...
""",
    ...
    if not args.skip_dusk:
        essential_cmds.append(
            "npm"
        )  # Adiciona npm como essencial se Dusk não for pulado
    ...
    if not ARTISAN_FILE.is_file():
        log(
            f"Erro Fatal: Arquivo 'artisan' não encontrado em {BASE_DIR}. Execute da raiz do projeto."
        )
        return 1
    if not VENDOR_DIR.is_dir():
        log(
            "Erro Fatal: Diretório 'vendor' não encontrado. Execute 'composer install'."
        )
        return 1
    ...
    if not args.skip_phpunit:
        ...
        if not phpunit_env_file.is_file() and PHPUNIT_ENV == "testing":
            log(
                f"  Aviso: Arquivo '{phpunit_env_file.name}' não encontrado. Usando config padrão/.env."
            )
    ...
    if not args.skip_dusk:
        ...
        if not DUSK_ENV_FILE.is_file():
            log(
                f"  Aviso: Arquivo '{DUSK_ENV_FILE.name}' não encontrado. Usando config padrão."
            )
        if not DUSK_TEST_DIR.is_dir():
            log(
                f"  Erro: Diretório '{DUSK_TEST_DIR.relative_to(BASE_DIR)}' não encontrado. Execute 'php artisan dusk:install'?"
            )
        ...
        if not chromedriver_path or not chromedriver_path.is_file():
            log(
                f"  Erro: ChromeDriver não encontrado em '{VENDOR_DIR / 'laravel' / 'dusk' / 'bin'}'. Execute 'php artisan dusk:chrome-driver --detect'?"
            )
            dusk_setup_failed = True
        ...
        if not (BASE_DIR / "node_modules").is_dir():
            log(
                "  Erro: Diretório 'node_modules' não encontrado. Execute 'npm install'."
            )
            dusk_setup_failed = True

        if dusk_setup_failed:
            log(
                "Erro: Falha na configuração/pré-requisitos do Dusk. Pulando testes Dusk."
            )
            if overall_exit_code == 0:
                overall_exit_code = 1  # Marca como erro
            if args.stop_on_failure:
                log(
                    "Parando execução devido à falha na configuração do Dusk e --stop-on-failure."
                )
                return overall_exit_code
        else:
            ...
            try:  # Envolve o setup e execução do Dusk em try para garantir cleanup
                ...
                log(
                    f"[Dusk] Iniciando ChromeDriver ({chromedriver_path.name}) na porta {DUSK_CHROMEDRIVER_PORT}..."
                )
                ...
                if not dusk_setup_failed:
                    log(
                        f"[Dusk] Iniciando servidor da aplicação na porta {DUSK_APP_PORT} (Ambiente: {DUSK_ENV})..."
                    )
                    serve_cmd = [
                        PHP_CMD,
                        str(ARTISAN_FILE),
                        "serve",
                        f"--port={DUSK_APP_PORT}",
                        f"--env={DUSK_ENV}",
                    ]
                    app_server_proc = start_background_process(
                        serve_cmd, DUSK_SERVE_LOG
                    )
                ...
                if not dusk_setup_failed:
                    ...
                    log(
                        "  Aguardando inicialização do Vite (10s)..."
                    )  # Aumentado para 10s
                    time.sleep(10)
                ...
                if not dusk_setup_failed:
                    ...
                    dusk_env_vars["APP_ENV"] = (
                        DUSK_ENV  # Garante que o Artisan use o env correto
                    )
                    ...
                    if exit_code_dusk != 0:
                        log(
                            f"Erro: Testes Dusk falharam (Código de Saída: {exit_code_dusk}). Verifique logs/screenshots."
                        )
                        overall_exit_code = exit_code_dusk
                        if args.stop_on_failure:
                            log(
                                "Parando execução devido à falha nos testes Dusk e --stop-on-failure."
                            )
                            return overall_exit_code  # Sai dentro do try
                    else:
                    ...
                    log(
                        "[Dusk] Pulando execução dos testes devido a falha no setup anterior."
                    )
