...
def copy_selected_files():
    ...
    try:
        ...
        print(f"Erro: Falha ao decodificar JSON do arquivo '{input_file_path}'. Verifique o formato.")
        return
    ...
    if not isinstance(relevant_files, list):
        print("Erro: A chave 'relevant_files' não foi encontrada ou não é uma lista no JSON.")
        return
    ...
    for relative_path_str in relevant_files:
        ...
        normalized_relative_path = Path(os.path.normpath(relative_path_str.lstrip('/\\')))
        ...
        if not source_file_path.is_file():
            print(f"  [AVISO] Arquivo fonte não encontrado ou não é um arquivo: '{source_file_path}'. Pulando.")
            skipped_count += 1
            continue
        ...
        try:
            ...
            print(f"  [ERRO] Falha ao copiar '{source_file_path}' para '{destination_file_path}': {e}")
            error_count += 1
