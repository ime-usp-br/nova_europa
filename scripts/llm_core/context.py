...
def _truncate_content(
    content: str, target_token_count: int, verbose: bool = False
) -> Tuple[str, int]:
    """
    ...
    if len(content) <= target_char_count:
        return content, (
            max(1, int(len(content) / chars_per_token_approx)) if content else 0
        )
    ...
    if target_char_count <= separator_len:
        ...
        new_token_estimate = (
            max(1, int(len(truncated_content_str) / chars_per_token_approx))
            if truncated_content_str
            else 0
        )
        if verbose:
            print(
                f"      Truncamento agressivo: mantendo {keep_start_chars} caracteres do início."
            )
    ...
    if verbose:
        ...
        print(
            f"      Conteúdo truncado: ~{new_token_estimate} tokens (original: ~{original_token_estimate} tokens)."
        )
...
def _load_files_from_dir(
    ...
    for filepath_abs in files_to_consider:
        ...
        if relative_path_str in skip_if_already_loaded_relative_paths:
            ...
            if verbose:
                print(
                    f"      - Pulando '{relative_path_str}' (já carregado como essencial)."
                )
            continue
        ...
        if relative_path_str in exclude_set:
            ...
            if verbose:
                print(
                    f"      - Excluindo '{relative_path_str}' (de _load_files_from_dir) devido a --exclude-context."
                )
            continue
        ...
        try:
            ...
            if (
                ...
                if isinstance(metadata, dict):
                    ...
                    if (
                        isinstance(manifest_token_count, int)
                        and manifest_token_count >= 0
                    ):
...
def get_essential_files_for_task(
    ...
    if verbose:
        print(
            f"  AC5.1a: Iniciando identificação de arquivos essenciais para tarefa '{task_name}'..."
        )
        if "args" in task_map:
            for arg_name, file_pattern_template in task_map["args"].items():
                print(
                    f"    - Padrão por argumento '{arg_name}': {file_pattern_template}"
                )
            ...
            if hasattr(cli_args, arg_name):
                ...
                if arg_value:
                    ...
                    replacements: Dict[str, str] = {
                        "latest_dir_name": latest_dir_name or ""
                    }
                    ...
                    if arg_name == "doc_file" and file_pattern_template == "{doc_file}":
                        ...
                        if not path_to_check.is_absolute():
                            path_to_check = (
                                core_config.PROJECT_ROOT / path_to_check
                            ).resolve(strict=False)
                        essential_file_abs_paths.append(path_to_check)
                        if verbose:
                            print(
                                f"    + Potencialmente essencial (de arg '{arg_name}'): {path_to_check.relative_to(core_config.PROJECT_ROOT) if path_to_check.is_absolute() and path_to_check.is_relative_to(core_config.PROJECT_ROOT) else path_to_check}"
                            )
                        continue

                    for key_placeholder, val_placeholder in replacements.items():
                        formatted_file_pattern = formatted_file_pattern.replace(
                            f"{{{key_placeholder}}}", val_placeholder
                        )

                    if (
                        "{" in formatted_file_pattern
                        and "}" in formatted_file_pattern
                        and verbose
                    ):
                        print(
                            f"    - Aviso: Placeholder não resolvido em '{formatted_file_pattern}' para arg '{arg_name}'. Pulando."
                        )
                        continue

                    abs_path = (
                        core_config.PROJECT_ROOT / formatted_file_pattern
                    ).resolve(strict=False)
                    essential_file_abs_paths.append(abs_path)
                    if verbose:
                        print(
                            f"    + Potencialmente essencial (de arg '{arg_name}'): {abs_path.relative_to(core_config.PROJECT_ROOT) if abs_path.is_absolute() and abs_path.is_relative_to(core_config.PROJECT_ROOT) else abs_path}"
                        )
    ...
    if "static" in task_map:
        for static_file_template in task_map["static"]:
            ...
            for key_placeholder, val_placeholder in replacements.items():
                formatted_static_file = formatted_static_file.replace(
                    f"{{{key_placeholder}}}", val_placeholder
                )

            if (
                "{" in formatted_static_file
                and "}" in formatted_static_file
                and verbose
            ):
                print(
                    f"    - Aviso: Placeholder não resolvido em '{formatted_static_file}' para arquivo estático. Pulando."
                )
                continue

            abs_path = (core_config.PROJECT_ROOT / formatted_static_file).resolve(
                strict=False
            )
            essential_file_abs_paths.append(abs_path)
            if verbose:
                print(
                    f"    + Potencialmente essencial (estático): {abs_path.relative_to(core_config.PROJECT_ROOT) if abs_path.is_absolute() and abs_path.is_relative_to(core_config.PROJECT_ROOT) else abs_path}"
                )

    unique_paths = sorted(list(set(essential_file_abs_paths)))
    if verbose:
        print(
            f"  AC5.1a: Lista final de {len(unique_paths)} caminhos de arquivos potencialmente essenciais identificados para '{task_name}':"
        )
...
def load_essential_files_content(
    ...
    if verbose and essential_file_paths_abs:
        print(
            f"  AC5.1b: Carregando conteúdo de {len(essential_file_paths_abs)} arquivo(s) essencial(is) (limite est. payload: {max_tokens_for_essentials_payload} tokens):"
        )
    ...
    for file_path_abs in essential_file_paths_abs:
        if current_tokens_estimate >= max_tokens_for_essentials_payload:
            if verbose:
                print(
                    f"    AC5.1b: Limite de tokens para payload essencial ({max_tokens_for_essentials_payload}) atingido. Pulando arquivos restantes."
                )
            break
        ...
        try:
            ...
            if verbose:
                print(
                    f"    Aviso (AC4.1): Arquivo essencial '{file_path_abs}' não parece estar dentro do PROJECT_ROOT. Pulando.",
                    file=sys.stderr,
                )
            continue

        if not file_path_abs.is_file():
            user_wants_to_continue = io_utils.prompt_user_for_missing_essential_file(
                relative_path_str
            )
            if not user_wants_to_continue:
                ...
                if verbose:
                    print(
                        f"    AC5.1b: Continuando sem o arquivo essencial ausente '{relative_path_str}'."
                    )
                continue

        try:
            content = file_path_abs.read_text(encoding="utf-8", errors="ignore")
            estimated_tokens_current_file = (
                max(1, int(len(content) / 3.8)) if content else 0
            )
            ...
            if (
                current_tokens_estimate + estimated_tokens_current_file
                > max_tokens_for_essentials_payload
            ):
                remaining_token_budget = (
                    max_tokens_for_essentials_payload - current_tokens_estimate
                )
                if remaining_token_budget < 50:
                    if verbose:
                        print(
                            f"    AVISO (AC3.4 / AC5.1b): Arquivo essencial '{relative_path_str}' ({estimated_tokens_current_file} tokens est.) pulado. Orçamento restante ({remaining_token_budget} tokens) muito pequeno para conteúdo útil."
                        )
                    ...
                    if verbose:
                        print(
                            f"    AVISO (AC3.4 / AC5.1b): Conteúdo do arquivo essencial '{relative_path_str}' ({estimated_tokens_current_file} tokens est.) será truncado para caber no orçamento de {remaining_token_budget} tokens."
                        )
                    content_to_add, tokens_to_add = _truncate_content(
                        content, remaining_token_budget, verbose
                    )
                    was_truncated = True
            ...
            if verbose:  # AC5.1b Log
                print(
                    f"    AC5.1b: Arquivo essencial '{relative_path_str}': Conteúdo {status_log_msg_part} (estimativa: {tokens_to_add} tokens)."
                )
            ...
            if tokens_to_add > 0 or not content_to_add:
                ...
                print(
                    f"    AC5.1b: Arquivo essencial '{relative_path_str}' resultou em 0 tokens após tentativa de truncamento e não foi adicionado ao payload da LLM seletora."
                )

        except Exception as e:
            if verbose:
                print(
                    f"  Aviso: Não foi possível ler ou processar o arquivo essencial '{file_path_abs.name}': {e}",
                    file=sys.stderr,
                )
...
def prepare_payload_for_selector_llm(
    ...
    essential_file_abs_paths = get_essential_files_for_task(
        task_name, cli_args, latest_dir_name, verbose
    )

    essential_content_str, loaded_essential_relative_paths = (
        load_essential_files_content(
            essential_file_abs_paths, max_tokens_for_essentials_payload, verbose
        )
    )
    ...
    loaded_essential_relative_paths_str_set = {
        p.as_posix() for p in loaded_essential_relative_paths
    }
    ...
    for path_str, metadata in files_metadata_from_manifest.items():
        if path_str not in loaded_essential_relative_paths_str_set:
            ...
            if token_c is None:
                ...
                if verbose:
                    print(
                        f"    - Aviso: token_count para '{path_str}' não é um inteiro ({token_c}), tratando como muito grande."
                    )
                pass
            else:
                token_c_val = token_c
            ...
            if token_c_val <= core_config.MANIFEST_MAX_TOKEN_FILTER:
                ...
                print(
                    f"    - Arquivo '{path_str}' (tokens: {token_c_val}) filtrado do manifesto para LLM seletora (limite: {core_config.MANIFEST_MAX_TOKEN_FILTER})."
                )

    remaining_manifest_json_str = json.dumps(
        {"files": remaining_manifest_files_for_selector}, indent=2, ensure_ascii=False
    )
    ...
    if verbose:
        essential_tokens_est = (
            max(1, int(len(essential_content_str) / 3.8))
            if essential_content_str
            else 0
        )
        remaining_manifest_tokens_est = (
            max(1, int(len(remaining_manifest_json_str) / 3.8))
            if remaining_manifest_json_str
            else 0
        )
        # Estima tokens do template
        prompt_template_tokens_est = max(
            1, int(len(re.sub(r"{{.*?}}", "", selector_prompt_template_content)) / 3.8)
        )

        total_payload_tokens_est = (
            essential_tokens_est
            + remaining_manifest_tokens_est
            + prompt_template_tokens_est
        )
        print(
            f"    Conteúdo essencial injetado no prompt seletor (~{essential_tokens_est} tokens)."
        )
        print(
            f"    JSON do manifesto dos demais arquivos (~{remaining_manifest_tokens_est} tokens, {len(remaining_manifest_files_for_selector)} arquivos)."
        )
        print(
            f"    Tamanho do prompt seletor template (sem placeholders) (~{prompt_template_tokens_est} tokens)."
        )
        print(
            f"    AC5.1c: Tamanho total estimado do payload para LLM seletora: ~{total_payload_tokens_est} tokens."
        )
...
def prepare_context_parts(
    ...
    if verbose:
        print("  Carregando e processando arquivos de contexto...")
    ...
    if task_name_for_essentials and cli_args_for_essentials:
        ...
        for p in essential_file_abs_paths:
            try:
                essential_map_paths_relative_str.add(
                    p.relative_to(core_config.PROJECT_ROOT).as_posix()
                )
            except ValueError:
                if verbose:
                    print(
                        f"      - Aviso (Map Essencial): Arquivo '{p}' fora da raiz. Não será marcado como essencial pelo mapa."
                    )

        for essential_abs_path in essential_file_abs_paths:
            try:
                rel_path_str = essential_abs_path.relative_to(
                    core_config.PROJECT_ROOT
                ).as_posix()
            except ValueError:
                if verbose:
                    print(
                        f"      - Aviso (Essencial): Arquivo '{essential_abs_path}' fora da raiz. Pulando."
                    )
                continue
            ...
            if not essential_abs_path.is_file():
                if not io_utils.prompt_user_for_missing_essential_file(rel_path_str):
                    raise MissingEssentialFileAbort(
                        f"Tarefa abortada pelo usuário: arquivo essencial '{rel_path_str}' ausente."
                    )
                if verbose:
                    print(
                        f"      - Aviso (Essencial): '{rel_path_str}' não encontrado, continuando sem ele por escolha do usuário."
                    )
                continue

            try:
                content = essential_abs_path.read_text(
                ...
                if (
                    manifest_data
                    and "files" in manifest_data
                    and rel_path_str in manifest_data["files"]
                ):
                    ...
                    if isinstance(meta, dict):
                        ...
                        if (
                            isinstance(stc_from_manifest, int)
                            and stc_from_manifest >= 0
                        ):
                        ...
                        if isinstance(manifest_token_count_val, int):
                            token_count_val = manifest_token_count_val
                ...
                processed_units.append(
                    FileProcessUnit(
                        ...
                        original_token_count=token_count_val,
                        file_type=type_val,
                        summary=summary_val,
                        summary_token_count=summary_token_val,
                ...
                if verbose:
                    print(
                        f"    + Carregado como essencial: '{rel_path_str}' ({token_count_val} tokens)."
                    )
            except Exception as e:
                if verbose:
                    print(
                        f"      - Aviso (Essencial): Não foi possível ler '{rel_path_str}': {e}",
                        file=sys.stderr,
                    )
    ...
    if include_list is not None:
        ...
        for rel_path_str_incl in include_list:
            if rel_path_str_incl in loaded_as_essential_paths_str:
                ...
                if verbose:
                    print(
                        f"      - Pulando '{rel_path_str_incl}' (já carregado como essencial)."
                    )
                continue
            if rel_path_str_incl in exclude_set:
                ...
                if verbose:
                    print(
                        f"      - Excluindo '{rel_path_str_incl}' (de include_list) por --exclude-context."
                    )
                continue

            filepath_abs_incl = (core_config.PROJECT_ROOT / rel_path_str_incl).resolve(
                strict=False
            )
            if not filepath_abs_incl.is_file():
                if verbose:
                    print(
                        f"      - Aviso: Arquivo incluído '{rel_path_str_incl}' não encontrado. Pulando."
                    )
            ...
            try:
                ...
                if verbose:
                    print(
                        f"      - Aviso: Pulando arquivo fora da raiz: {rel_path_str_incl}",
                        file=sys.stderr,
                    )
                skipped_not_found_incl += 1
                continue
            ...
            try:
                ...
                original_token_count = max(1, int(len(content) / 3.8)) if content else 0
                summary_text_incl, summary_tokens_val_incl, file_type_val_incl = (
                    None,
                    None,
                    None,
                )
                ...
                if (
                    manifest_data
                    and "files" in manifest_data
                    and rel_path_str_incl in manifest_data["files"]
                ):
                    ...
                    if isinstance(metadata, dict):
                        ...
                        if (
                            isinstance(stc_from_manifest_incl, int)
                            and stc_from_manifest_incl >= 0
                        ):
                            summary_tokens_val_incl = stc_from_manifest_incl
                        elif summary_text_incl:
                            summary_tokens_val_incl = max(
                                1, int(len(summary_text_incl) / 3.8)
                            )
                        else:
                            summary_tokens_val_incl = None
                        file_type_val_incl = metadata.get("type")
                        manifest_token_count_val_incl = metadata.get("token_count")
                        if isinstance(manifest_token_count_val_incl, int):
                            original_token_count = manifest_token_count_val_incl

                processed_units.append(
                    FileProcessUnit(
                        relative_path=rel_path_str_incl,
                        content=content,
                        original_content=content,
                        token_count=original_token_count,
                        original_token_count=original_token_count,
                        file_type=file_type_val_incl,
                        summary=summary_text_incl,
                ...
                if verbose:
                    print(
                        f"      - Aviso: Não foi possível ler '{rel_path_str_incl}': {e}",
                        file=sys.stderr,
                    )
                skipped_not_found_incl += 1
        if verbose:
            print(f"    Carregados {loaded_count_incl} arquivos da lista de inclusão.")
            if skipped_already_loaded_incl > 0:
                print(
                    f"    Pulados {skipped_already_loaded_incl} da include_list (já carregados como essenciais)."
                )
            if exclude_set:
                print(
                    f"    Excluídos {excluded_by_arg_count_incl} da include_list por --exclude-context."
                )
            if skipped_not_found_incl > 0:
                print(
                    f"    Pulados {skipped_not_found_incl} da include_list (não encontrados/legíveis)."
                )
    else:
        if verbose:
            print(
                "    Carregando de diretórios padrão (contexto mais recente + comum)..."
            )
        if primary_context_dir:
            _load_files_from_dir(
                ...
                exclude_set,
                manifest_data,
                essential_map_paths_relative_str,
                loaded_as_essential_paths_str,
                verbose,
            )
        if (
            common_context_dir
    ...
    if (
        max_input_tokens_for_call is not None
        and current_total_tokens > max_input_tokens_for_call
    ):
        if verbose:
            print(
                f"  AVISO (AC2.2): Contexto inicial ({current_total_tokens} tokens) excede o limite ({max_input_tokens_for_call} tokens). Aplicando reduções..."
            )
        ...
        units_for_summary_reduction = sorted(
            [
                ...
                if not unit.is_essential_from_map
                and unit.summary
                and unit.summary_token_count is not None
                and unit.summary_token_count < unit.original_token_count
            ],
            key=lambda u: (
                u.original_token_count
                - (u.summary_token_count or u.original_token_count)
            ),
            reverse=True,
        )
        for unit in units_for_summary_reduction:
            ...
            if unit.summary and unit.summary_token_count is not None:
                ...
                if token_reduction > 0:
                    if verbose:
                        print(
                            f"    AC2.2.1: Substituindo '{unit.relative_path}' ({unit.original_token_count} tokens originais) por sumário ({unit.summary_token_count} tokens). Economia: {token_reduction}"
                        )
        ...
        if current_total_tokens > max_input_tokens_for_call:
            units_to_truncate_non_essential = sorted(
                [
                    unit
                    for unit in processed_units
                    if not unit.is_essential_from_map and not unit.is_reduced_to_summary
                ],
                key=lambda u: u.token_count,
                reverse=True,
            )
            for unit in units_to_truncate_non_essential:
                if current_total_tokens <= max_input_tokens_for_call:
                    break
                needed_reduction_overall = (
                    current_total_tokens - max_input_tokens_for_call
                )
                max_reduction_for_this_file = max(0, unit.token_count - 50)
                reduction_to_apply = min(
                    needed_reduction_overall, max_reduction_for_this_file
                )
                if reduction_to_apply > 0:
                    ...
                    unit.content, unit.token_count = _truncate_content(
                        unit.original_content, target_token_for_this_file, verbose
                    )
                    ...
                    if verbose:
                        print(
                            f"    AC2.2.2 (Não Essencial): Truncando '{unit.relative_path}' de {original_tokens_before_trunc} para {unit.token_count} tokens."
                        )
        ...
        if current_total_tokens > max_input_tokens_for_call:
            units_to_truncate_essential = sorted(
                [
                    unit
                    for unit in processed_units
                    if unit.is_essential_from_map and not unit.is_reduced_to_summary
                ],
                key=lambda u: u.token_count,
                reverse=True,
            )
            for unit in units_to_truncate_essential:
                ...
                if reduction_to_apply > 0:
                    ...
                    if verbose:
                        print(
                            f"    AVISO (AC3.4): Conteúdo do arquivo essencial '{unit.relative_path}' ({original_tokens_before_trunc} tokens est.) foi truncado para {unit.token_count} tokens para caber no limite da chamada principal."
                        )

    for unit in processed_units:
        content_type_log = "integral"
        if unit.is_reduced_to_summary:
            content_type_log = "sumário"
        elif unit.is_truncated:
            content_type_log = "truncado"

        if verbose:
            print(
                f"    -> Incluindo '{unit.relative_path}' ({unit.token_count} tokens) como conteúdo {content_type_log}."
            )

        summary_block_for_part_text = ""
        if (
            unit.summary and not unit.is_reduced_to_summary and not is_for_selector_llm
        ):  # is_for_selector_llm condition is not relevant here
            summary_block_for_part_text = (
                f"--- SUMMARY ---\n{unit.summary}\n--- END SUMMARY ---"
            )

        delimiter_start = (
            core_config.ESSENTIAL_CONTENT_DELIMITER_START
            if unit.is_essential_from_map
            else core_config.SUMMARY_CONTENT_DELIMITER_START
        )
        delimiter_end = (
            core_config.ESSENTIAL_CONTENT_DELIMITER_END
            if unit.is_essential_from_map
            else core_config.SUMMARY_CONTENT_DELIMITER_END
        )
    ...
    if (
        verbose
        and max_input_tokens_for_call
        and final_total_tokens > max_input_tokens_for_call
    ):
        print(
            f"  AVISO FINAL (AC2.2): Contexto final ({final_total_tokens} tokens) ainda excede o limite ({max_input_tokens_for_call} tokens) após reduções."
        )

    print(
        f"  Total de partes de contexto preparadas: {len(context_parts_final)} (~{final_total_tokens} tokens)."
    )
...
def confirm_and_modify_selection(
    ...
    if verbose:
        print(
            f"  AC5.1d: Lista de arquivos retornada pela LLM seletora ({len(current_files)} arquivos):"
        )
    ...
    def display_current_selection():
        ...
        if not current_files:
            ...
            for i, filepath_str in enumerate(current_files):
                ...
                token_count_str = (
                    str(token_count_val) if token_count_val is not None else "N/A"
                )
    ...
    while True:
        ...
        if choice.lower() in ["y", "yes", ""]:
            if verbose:
                print(
                    f"  Usuário confirmou o uso de {len(current_files)} arquivos selecionados para o contexto."
                )
            return current_files
        elif choice.lower() in ["n", "no"]:
            if verbose:
                print("  Usuário optou por usar o contexto padrão em vez da seleção.")
            return None
        elif choice.lower() in ["q", "quit"]:
            if verbose:
                print("  Tarefa abortada pelo usuário durante a seleção de contexto.")
            sys.exit(0)
            ...
            try:
                ...
                if path_to_add_relative_str not in current_files:
                    ...
                    if verbose:
                        print(
                            f"    AC5.1e: Usuário adicionou '{path_to_add_relative_str}' à seleção."
                        )
                else:
            ...
            try:
                ...
                if 0 <= index_to_remove < len(current_files):
                    ...
                    print(
                        f"  Removed '{original_path_removed_for_log}' (index {index_to_remove + 1})."
                    )
            ...
            if removed_successfully and verbose and original_path_removed_for_log:
                print(
                    f"    AC5.1e: Usuário removeu '{original_path_removed_for_log}' da seleção."
                )
