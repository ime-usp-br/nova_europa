...
def find_latest_context_dir(context_base_dir: Path) -> Optional[Path]:
    ...
    if not context_base_dir.is_dir():
        print(f"Erro: Diretório base de contexto não encontrado: {context_base_dir}", file=sys.stderr)
        return None

    valid_context_dirs = [
        d for d in context_base_dir.iterdir()
        if d.is_dir() and re.match(core_config.TIMESTAMP_DIR_REGEX, d.name)
    ]

    if not valid_context_dirs:
        print(f"Erro: Nenhum diretório de contexto válido encontrado em {context_base_dir}", file=sys.stderr)
        return None
...
def _load_files_from_dir(
    ...
    for filepath in files_to_consider:
        ...
        try:
            ...
            relative_path_str = str(resolved_filepath.relative_to(core_config.PROJECT_ROOT).as_posix())
        except (FileNotFoundError, ValueError) as e:
            print(f"      - Aviso: Pulando arquivo {filepath.name} devido a erro de caminho: {e}", file=sys.stderr)
            continue
        ...
        try:
            ...
            print(f"      - Aviso: Não foi possível ler o arquivo {relative_path_str}: {e}", file=sys.stderr)

    print(f"      - Carregados {loaded_count} arquivo(s) de {context_dir.relative_to(core_config.PROJECT_ROOT)} (carregamento padrão).")
    if exclude_list:
        print(f"      - Excluídos {excluded_count} arquivo(s) com base em --exclude-context (carregamento padrão).")
...
def prepare_context_parts(
    ...
    if include_list is not None:
        print(f"    Carregando com base na lista de inclusão ({len(include_list)} arquivos)...")
        include_set = set(include_list)

        for relative_path_str in include_set:
            if relative_path_str in exclude_set:
                excluded_by_arg_count += 1
                print(f"      - Excluindo '{relative_path_str}' devido a --exclude-context.")
                continue

            filepath_absolute = (core_config.PROJECT_ROOT / relative_path_str).resolve(strict=False)

            if not filepath_absolute.is_file():
                print(f"      - Aviso: Arquivo incluído não encontrado ou não é um arquivo: {relative_path_str}", file=sys.stderr)
                skipped_not_found += 1
                continue
            try:
                filepath_absolute.relative_to(core_config.PROJECT_ROOT)
            except ValueError:
                print(f"      - Aviso: Pulando arquivo fora da raiz do projeto: {relative_path_str}", file=sys.stderr)
                skipped_not_found += 1
                continue
            try:
        ...
        if exclude_list:
            print(f"    Excluídos {excluded_by_arg_count} arquivos com base em --exclude-context.")
        if skipped_not_found > 0:
            print(f"    Pulados {skipped_not_found} arquivos incluídos (não encontrados/legíveis/fora do projeto).")
...
def find_latest_manifest_json(manifest_data_dir: Path) -> Optional[Path]:
    ...
    if not manifest_data_dir.is_dir():
        return None
    manifest_files = [
        f for f in manifest_data_dir.glob("*_manifest.json")
...
def load_manifest(manifest_path: Path) -> Optional[Dict[str, Any]]:
    ...
    if not manifest_path.is_file():
        print(f"Erro: Arquivo de manifesto não encontrado: {manifest_path}", file=sys.stderr)
        return None
    try:
        with open(manifest_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        if not isinstance(data, dict) or "files" not in data or not isinstance(data["files"], dict):
            print(f"Erro: Formato inválido no manifesto {manifest_path.name}. Chave 'files' ausente/inválida.", file=sys.stderr)
            return None
        return data
        ...
        print(f"Erro ao ler arquivo de manifesto {manifest_path.name}: {e}", file=sys.stderr)
        return None
...
def confirm_and_modify_selection(
    ...
    def display_current_selection():
        ...
        if not current_files:
            ...
            if max_input_tokens:
                print(f"  Recommended Max Input Tokens for API call: {max_input_tokens}")
                if total_tokens > max_input_tokens:
                    print(f"  WARNING: Current selection ({total_tokens}) exceeds recommended max ({max_input_tokens})!")
        print("------------------------------------")

    while True:
        display_current_selection()
        print("Commands: 'y' (yes/confirm), 'n' (no/use default context), 'a path/to/add' (add file), 'r index|path' (remove file), 'q' (quit task)")
        choice = input("Confirm selection, modify, or use default context? [Y]: ").strip().lower()

        if choice in ["y", "yes", ""]:
            print(f"  User confirmed using {len(current_files)} selected files for context.")
            ...
            abs_path_to_add = (core_config.PROJECT_ROOT / path_to_add).resolve(strict=False)
            if not abs_path_to_add.is_file():
                 print(f"  Error: File to add '{path_to_add}' does not exist or is not a file.")
                 continue
            try:
                path_to_add_relative_str = str(abs_path_to_add.relative_to(core_config.PROJECT_ROOT).as_posix())
                if path_to_add_relative_str not in current_files:
                 ...
                 print(f"  Error: Path to add '{path_to_add}' seems to be outside the project root. Cannot add.")
            ...
            try:
                ...
                if path_to_remove_normalized in current_files:
                    ...
                    for i, p_str in reversed(list(enumerate(current_files))): # Iterate reversed to pop safely
                        if p_str.endswith(path_to_remove_normalized):
                            removed_path = current_files.pop(i)
                            print(f"  Removed '{removed_path}' (matched ending with '{path_to_remove_normalized}').")
                            removed_successfully = True
                            found_partial_match = True
                            break # Remove only the first found partial match
                    if not found_partial_match:
                         print(f"  Error: Path '{path_to_remove_normalized}' not found in the list for removal.")

            if not removed_successfully and not item_to_remove_str.isdigit():
                 print(f"  Error: Could not remove '{item_to_remove_str}' (not a valid index or existing path/suffix).")
        else:
