...
def save_llm_response(
    ...
    current_output_dir_base = output_dir_base_override if output_dir_base_override is not None else core_config.OUTPUT_DIR_BASE
...
def parse_pr_content(llm_output: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        title_start_index = llm_output.index(title_start_delimiter) + len(title_start_delimiter)
        ...
        if not title or body is None: # Body pode ser string vazia, mas não None se delimitador presente
