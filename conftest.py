...
def pytest_collection_modifyitems(config, items):
    ...
    if not run_live_tests:
        ...
        for item in items:
            if (
                "live" in item.keywords and "test_create_issue" in item.nodeid
            ):  # Escopo para create_issue
...
def test_repo(request):  # Adiciona request para usar config
    ...
    if request.config.getoption("--live") and not repo_env_var:
        ...
        pytest.fail(
            "GH_TEST_REPO environment variable not set, but --live flag was used. Required for live GitHub tests."
        )
    if repo_env_var and ("/" not in repo_env_var or len(repo_env_var.split("/")) != 2):
        pytest.fail(
            f"Invalid GH_TEST_REPO format: '{repo_env_var}'. Expected 'owner/repo'."
        )
...
def change_project_root(tmp_path: Path, monkeypatch):
    ...
    if "scripts.update_project" in sys.modules:
        original_update_project_base_dir = sys.modules[
    ...
    if "scripts.create_issue" in sys.modules:
        ...
        if hasattr(
            sys.modules["scripts.create_issue"], "TEMPLATE_DIR"
        ):  # Verificar se existe
            original_create_issue_template_dir = sys.modules[
                "scripts.create_issue"
            ].TEMPLATE_DIR
            monkeypatch.setattr(
                sys.modules["scripts.create_issue"],
                "TEMPLATE_DIR",
                tmp_path / "templates" / "issue_bodies",
            )
    ...
    if "scripts.update_project" in sys.modules and original_update_project_base_dir:
        monkeypatch.setattr(
            sys.modules["scripts.update_project"],
            "BASE_DIR",
            original_update_project_base_dir,
        )
    if "scripts.create_issue" in sys.modules:
        if original_create_issue_base_dir:
            monkeypatch.setattr(
                sys.modules["scripts.create_issue"],
                "BASE_DIR",
                original_create_issue_base_dir,
            )
        if (
            original_create_issue_template_dir
        ):  # Garante que existia antes de tentar restaurar
            monkeypatch.setattr(
                sys.modules["scripts.create_issue"],
                "TEMPLATE_DIR",
                original_create_issue_template_dir,
