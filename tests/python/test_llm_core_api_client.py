...
def test_load_api_keys_no_env_var_but_dotenv_has_it(mock_dotenv_load_fixture):
    ...
    with patch('scripts.llm_core.api_client.os.getenv', side_effect=getenv_side_effect) as mock_os_getenv:
        assert api_client.load_api_keys(verbose=True) is True 
        assert api_client.api_key_loaded_successfully is True
        assert len(api_client.GEMINI_API_KEYS_LIST) == 2
...
def test_initialize_genai_client_success(mock_genai_client_constructor, mock_dotenv_load_fixture):
    api_client.api_key_loaded_successfully = False 
...
def test_execute_gemini_call_success(mock_executor, mock_client, mock_dotenv_load_fixture):
    mock_future = MagicMock()
    mock_response = MagicMock()
    mock_response.text = "Mocked LLM Response"
    mock_response.prompt_feedback = None 
    mock_response.candidates = [MagicMock(finish_reason=api_client.types.FinishReason.STOP)]
