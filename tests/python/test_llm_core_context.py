...
@patch("builtins.open", new_callable=mock_open, read_data='{"files": {"path/to/file.py": {"type": "code_python"}}}')
...
def test_load_manifest_invalid_json(mock_file_invalid_json, mock_path_is_file, tmp_path: Path):
...
def test_load_manifest_missing_files_key(mock_file_missing_key, mock_path_is_file, tmp_path: Path):
...
def test_prepare_context_parts_default_loading(mock_load_from_dir, tmp_path: Path, monkeypatch):
    ...
    core_context.prepare_context_parts(primary_dir, common_dir, exclude_list=None, manifest_data=None, include_list=None)
