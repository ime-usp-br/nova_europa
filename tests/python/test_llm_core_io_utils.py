...
def test_prompt_user_for_missing_essential_file_continue(mock_input: MagicMock, capsys):
    ...
    assert "AVISO (AC4.1a): Arquivo essencial 'missing.txt' não encontrado no disco. Deseja (C)ontinuar sem este arquivo ou (A)bortar a tarefa? [A]: " in captured.out


@patch("builtins.input")
def test_prompt_user_for_missing_essential_file_abort_explicit(mock_input: MagicMock, capsys):
...
@patch("builtins.input")
def test_prompt_user_for_missing_essential_file_abort_default(mock_input: MagicMock, capsys):
...
@patch("builtins.input")
def test_prompt_user_for_missing_essential_file_invalid_then_continue(mock_input: MagicMock, capsys):
    mock_input.side_effect = ["x", "c"] # Entrada inválida, depois 'c'
    result = io_utils.prompt_user_for_missing_essential_file("missing.txt")
    assert result is True
    captured = capsys.readouterr()
    assert "Entrada inválida. Por favor, digite 'C' para continuar ou 'A' para abortar." in captured.out
