...
def test_load_and_fill_template_basic_replacement(tmp_path: Path):
    ...
    assert core_prompts_module.load_and_fill_template(template_file, variables) == expected_output
...
def test_load_and_fill_template_no_placeholders(tmp_path: Path):
    ...
    assert core_prompts_module.load_and_fill_template(template_file, variables) == template_content
...
def test_modify_prompt_with_observation_adds_feedback():
    ...
    modified = core_prompts_module.modify_prompt_with_observation(original_prompt, observation)
...
def test_modify_prompt_with_no_observation():
    ...
    modified_empty = core_prompts_module.modify_prompt_with_observation(original_prompt, "")
...
def setup_selector_prompts_env(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(core_config, 'CONTEXT_SELECTORS_DIR', selectors_dir_absolute_in_tmp)
    
    (selectors_dir_absolute_in_tmp / "select-context-for-resolve-ac-1stage.txt").write_text("1stage resolve content")
    (selectors_dir_absolute_in_tmp / "select-context-for-resolve-ac-2stages.txt").write_text("2stages resolve content")
    (selectors_dir_absolute_in_tmp / "select-context-for-commit-mesage.txt").write_text("commit fallback content") 
    ...
    monkeypatch.setattr(core_config, 'CONTEXT_SELECTORS_DIR', original_context_selectors_dir)
...
def test_find_context_selector_prompt_fallback(setup_selector_prompts_env):
    ...
    # Teste com False
    found_path_false = core_prompts_module.find_context_selector_prompt("commit-mesage", False)
    assert found_path_false == expected_path
    # Teste com True
    found_path_true = core_prompts_module.find_context_selector_prompt("commit-mesage", True)
...
def test_find_context_selector_prompt_not_found(setup_selector_prompts_env):
    ...
    assert core_prompts_module.find_context_selector_prompt("non-existent-task", False) is None
    assert core_prompts_module.find_context_selector_prompt("non-existent-task", True) is None
