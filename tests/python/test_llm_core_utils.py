...
def test_run_command_success(mock_subprocess_run):
    ...
    exit_code, stdout, stderr = core_utils.run_command(cmd, cwd=core_config.PROJECT_ROOT, check=True)
...
def test_run_command_failure_check_true(mock_subprocess_run):
    ...
    mock_subprocess_run.side_effect = subprocess.CalledProcessError(
        returncode=1, cmd=["failing_cmd"], output="", stderr="Error output from CalledProcessError"
    )
...
@patch('subprocess.run', side_effect=subprocess.TimeoutExpired(cmd="timeout_cmd", timeout=10))
