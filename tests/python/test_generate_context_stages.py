...
@patch("scripts.generate_context.subprocess.run") # Este mock não será usado se run_command for mockado
def test_stages_argument_parsing(mock_subprocess_run, mock_shutil_which, tmp_path, mock_args, capsys):
    ...
    monkeypatch.setattr(generate_context, "DEFAULT_OUTPUT_BASE_DIR", mock_default_output_base_dir)
...
@patch("scripts.generate_context.invoke_manifest_generator") # Mock para não executar o script real
...
def test_fallback_logic_for_skipped_stages(
    ...
    mock_run_command_func.return_value = (0, "Mocked manifest gen stdout", "Mocked manifest gen stderr")

    args_instance = mock_args(stages=["env"], output_dir=output_base_for_script, verbose=True) 
    ...
    with patch.dict(generate_context.STAGES_CONFIG, mocked_stages_config_for_test):
         generate_context.run_all_collections(current_run_dir, current_run_timestamp, args_instance)

    mock_shutil_copy2.assert_any_call(previous_context_dir_mock / "git_log.txt", current_run_dir / "git_log.txt")
    mock_shutil_copy2.assert_any_call(previous_context_dir_mock / "git_status.txt", current_run_dir / "git_status.txt")
    
    captured = capsys.readouterr()
    assert f"AVISO (AC5.3): Arquivo esperado 'git_diff_cached.txt' do estágio 'git' não encontrado em '{previous_run_timestamp}'. Não copiado." in captured.out
    assert not (current_run_dir / "git_diff_cached.txt").exists()

    mock_find_second_latest.return_value = None 
    if (current_run_dir / "git_log.txt").exists(): (current_run_dir / "git_log.txt").unlink(missing_ok=True) # Python 3.8+
    if (current_run_dir / "git_status.txt").exists(): (current_run_dir / "git_status.txt").unlink(missing_ok=True)
    ...
    assert f"AVISO (AC6): Nenhum diretório de contexto anterior encontrado para fallback dos estágios pulados:" in captured_ac6.out
