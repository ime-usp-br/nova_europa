...
from scripts.llm_core import config as task_core_config # task_core_config é o alias usado no script
...
def test_check_new_commits_found(mock_run_command):
    ...
    mock_run_command.side_effect = [
        (0, "", ""),  # git fetch
        (0, "refs/remotes/origin/main", ""), # git show-ref (base branch exists remotely)
        (0, "3\n", "") # git rev-list --count
    ]
    assert llm_task_create_pr.check_new_commits("main", "feature/xyz", verbose=True) is True
    assert mock_run_command.call_count == 3
    mock_run_command.assert_any_call(["git", "fetch", "origin", "feature/xyz", "main"], check=False)
    mock_run_command.assert_any_call(["git", "show-ref", "--verify", "refs/remotes/origin/main"], check=False)
    mock_run_command.assert_any_call(["git", "rev-list", "--count", "origin/main..feature/xyz"], check=False)
...
def test_check_new_commits_not_found(mock_run_command):
    ...
    assert llm_task_create_pr.check_new_commits("main", "feature/xyz", verbose=True) is False


@patch("scripts.tasks.llm_task_create_pr.core_utils.command_exists", return_value=True) # Assume gh exists
@patch("scripts.tasks.llm_task_create_pr.core_utils.run_command")
def test_create_github_pr_success(mock_run_command, mock_command_exists_true): # Renomeado para evitar conflito
    ...
    mock_run_command.assert_called_once_with(
        ["gh", "pr", "create", "--title", "Test PR Title", "--body", "Test PR Body", "--head", "feature/xyz", "--base", "main"],
        check=False
    )

@patch("scripts.tasks.llm_task_create_pr.core_utils.command_exists", return_value=True) # Assume gh exists
@patch("scripts.tasks.llm_task_create_pr.core_utils.run_command")
def test_create_github_pr_draft_success(mock_run_command, mock_command_exists_true): # Renomeado
    ...
    assert success is True
    mock_run_command.assert_called_once_with(
        ["gh", "pr", "create", "--title", "Draft PR", "--body", "Draft Body", "--head", "feature/draft", "--base", "develop", "--draft"],
        check=False
    )


@patch("scripts.tasks.llm_task_create_pr.core_utils.command_exists")
def test_create_github_pr_gh_not_found(mock_command_exists_param): # Renomeado parâmetro
...
def test_main_create_pr_direct_flow_success(
    ...
    mock_parse_pr_content.return_value = ("PR Gerado", "Corpo do PR gerado.\n\nCloses #123")
    ...
    mock_create_gh_pr.assert_called_once_with("PR Gerado", "Corpo do PR gerado.\n\nCloses #123", "feature/123-my-feature", "main", False, True)
