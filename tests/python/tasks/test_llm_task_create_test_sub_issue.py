...
def test_add_task_specific_args():
    ...
    try:
        ...
        pytest.fail(f"Argumentos específicos da tarefa não foram corretamente definidos. Erro: {e}")

@patch("scripts.tasks.llm_task_create_test_sub_issue.core_args_module.get_common_arg_parser")
...
@patch("scripts.tasks.llm_task_create_test_sub_issue.core_context.prepare_context_parts")
@patch("scripts.tasks.llm_task_create_test_sub_issue.core_prompts_module.load_and_fill_template")
...
def test_main_create_test_sub_issue_direct_flow_success(
    ...
    mock_execute_gemini.return_value = "TITLE: Test Sub-Issue Content\nTYPE: test\n..." # Conteúdo simulado da issue
    ...
    monkeypatch.setattr(task_core_config, "META_PROMPT_DIR", tmp_path) # Também para o caso de --two-stage
    monkeypatch.setattr(task_core_config, "PROJECT_ROOT", tmp_path)

    (tmp_path / llm_task_create_test_sub_issue.PROMPT_TEMPLATE_NAME).write_text("Template content for test sub-issue")
    ...
    try:
        ...
        monkeypatch.setattr(task_core_config, "META_PROMPT_DIR", original_meta_prompt_dir)
    ...
    mock_save_response.assert_called_once_with(
        llm_task_create_test_sub_issue.TASK_NAME, mock_execute_gemini.return_value.strip()
    )
