...
def test_add_task_specific_args():
    ...
    try:
        ...
        pytest.fail(f"Argumentos específicos da tarefa não foram corretamente definidos como opcionais. Erro: {e}")
...
@patch("scripts.tasks.llm_task_commit_mesage.core_prompts_module.load_and_fill_template")
...
def test_main_commit_mesage_direct_flow_success(
    ...
    mock_load_template.return_value = "Template de commit preenchido para issue __NUMERO_DA_ISSUE__"
    ...
    (tmp_path / llm_task_commit_mesage.PROMPT_TEMPLATE_NAME).write_text("Template content for commit")
