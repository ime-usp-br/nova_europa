...
from scripts.llm_core import config as core_config_test_scope # Para restaurar depois, se necessário
    ...
    try:
        ...
        for action in parser._actions:
            if action.dest == 'issue' and action.required:
                 pytest.fail("Erro inesperado ao parsear com --issue e --ac, SystemExit ocorreu mas não deveria ser por falta destes.")
...
def test_main_resolve_ac_direct_flow_success(
    mock_shutdown_api, mock_confirm_step, mock_save_response, 
    mock_load_template, mock_prepare_context, mock_execute_gemini,
    mock_startup_api, mock_get_common_parser,
    tmp_path, monkeypatch # Adicionado monkeypatch
):
    ...
    args = argparse.Namespace(
        issue="1", ac="1", observation="obs", two_stage=False, verbose=False,
        web_search=False, generate_context=False, select_context=False,
        exclude_context=[], only_meta=False, only_prompt=False, yes=True, 
    )
    ...
    monkeypatch.setattr(task_core_config, 'TEMPLATE_DIR', tmp_path) # Agora TEMPLATE_DIR aponta para tmp_path
    monkeypatch.setattr(task_core_config, 'PROJECT_ROOT', tmp_path) # PROJECT_ROOT também aponta para tmp_path

    (tmp_path / llm_task_resolve_ac.RESOLVE_AC_PROMPT_TEMPLATE_NAME).write_text("Template content")
